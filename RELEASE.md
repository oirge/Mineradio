...
- 单分支 `codex/release-v2.0.3`，两个提交：`30b3ba5 chore(release): prepare v2.0.3` 与 `5bae4cf fix(release): write checksum into dist`。PR [#73](https://github.com/oirge/Mineradio/pull/73) 用 merge commit 合入 `main`（合并提交 `d576231`），PR [#74](https://github.com/oirge/Mineradio/pull/74) 再合 `5bae4cf`（合并提交 `44b39d5`）；`Verify` run `34443758418` / `34443829972` 过分支、`34444003101` 过 `main`，均通过。
- annotated tag `v2.0.3` = tag object `ac634a30ae3eb64c0bbe61ff5b1ba5c7391968e9`，指向 release commit `30b3ba5f64d1afc01c93e05f2bf48893fc8a7845`，tagger 时间 `2026-09-10T05:49:32Z`；GitHub API 复验该提交是 `origin/main` 祖先。
- **第一次 `Build and Release`（run `34442721960`）失败，卡在「Upload release assets」**：`Generate SHA256 checksums` 步骤里 `cd dist` 之后用裸相对名 `Mineradio-oirge-$version-SHA256SUMS.txt` 落盘，.NET 的当前工作目录与 PowerShell 位置不一致，清单没写进 `dist`，上传前的 `Test-Path` 于是抛「待上传资产缺失：dist/Mineradio-oirge-2.0.3-SHA256SUMS.txt」。`5bae4cf` 改成 `Join-Path (Get-Location)` 并加回归断言后重跑成功。
- `Build and Release` run `34444159148` 成功（`06:11:57Z` 起 `06:14:25Z` 止，2 分 28 秒），Windows x64 NSIS 在 GitHub Actions 远程构建。本轮没有在本机启动 Electron、没有关闭或重启正在使用的 Mineradio，也没有执行安装包。
- **这一版没有出现 electron-builder 的双草稿。** 工作流改成显式管理后，构建步骤带 `--publish never` 不再隐式建 Release，上传步骤只在「同 tag 无 Release」时创建一个草稿并复用；`gh release list` 复验 v2.0.3 只有一个 Release。
- 正式发布 Release `386043356`，标题 `Mineradio v2.0.3：SSA 歌词与发布链路加固`，`published_at` `2026-09-10T06:22:41Z`，非 draft / 非 prerelease；`releases/latest` 已复验指向 `v2.0.3`。
- 四项资产全部回下载到系统临时目录复算：

| 资产 | 字节 | SHA256 |
| --- | ---: | --- |
| `Mineradio-oirge-2.0.3-Setup.exe` | 102617620 | `3f3527dc7bd8c1d158fcfe57aab159c5ddf1addbacbc8ebc1db6aaf73494eb60` |
| `Mineradio-oirge-2.0.3-Setup.exe.blockmap` | 106795 | `c232dc214aae1eeb1d8a39d1a362717de99f28a78f53ffd9859248878baaa5e2` |
| `Mineradio-oirge-2.0.3-SHA256SUMS.txt` | 282 | `90c39e20ffff1352f6666384a00096125dbcdf1a3bd0150f129955c094c19ad5` |
| `latest.yml` | 359 | `0fb332de686523aea018dda0f111f9b038dab324382a81f9b3742296ccd3c43c` |

- 三路校验全部通过：GitHub API 的四个 `digest` 与本机 SHA256 逐字相同；SHA256 清单内安装器 / blockmap / `latest.yml` 三条全部匹配；`latest.yml` 的 `version` / `path` 为 `2.0.3` / `Mineradio-oirge-2.0.3-Setup.exe`，`files[0].size` `102617620` 与安装器字节数相同，文件项与顶层 SHA512 都是 `7Eps3b5zVEHgZOcbJVrl+gegYcBtNmuqtsPmMfWkcrXBGtGv3oEWZDzJCqyFjCAdeAUud74VuP3S26UoI3HvXA==`，与本机复算一致；`releaseDate` `2026-09-10T06:14:07.072Z`。
- **`SHA256SUMS.txt` 的 CRLF 遗留这一版修掉了**：清单 282 字节、3 行、LF 行尾、无 BOM，`sha256sum -c` 在 LF 环境下可直接跑，不再需要 `tr -d '\r'`。
- 资产记录分支 `docs/release-assets-v203-fix`（本条记录自己就在这个分支上，PR 与合并提交号下一版补记）。这个分支除了回填 v2.0.3 资产，还顺手修复了 `RELEASE.md` 自 v1.2.61 起被损坏的编码与下方历史小节，并给 `Verify` 加了文档编码门禁，详见 PR 描述。
...
- 单分支 `codex/release-v2.0.0`，两个提交：`9ab071e fix: 修复视觉预设 7/8 读档后被截回 6` 与 `ac242db chore(release): 2.0.0`。PR [#67](https://github.com/oirge/Mineradio/pull/67) 用 merge commit 合入 `main`，合并提交 `4127d22`；两路 `Verify` run `34078239457` / `34078268287` 均通过。
- annotated tag `v2.0.0` = tag object `f51c2a9b6d1f485c6a4b806e044a06097694ba09`，指向 release commit `ac242db66acab59488e8146ad954b1785986ee21`；该提交是 `origin/main` 的祖先。
- `Build and Release` run `34078489791` 成功（2 分 23 秒），Windows x64 NSIS 在 GitHub Actions 远程构建。用户要求后台测试不影响电脑，所以本轮没有在本机跑 electron-builder、没有启动 Electron、没有安装或执行下载回来的安装包。
- electron-builder 的双草稿第十三次复现：完整 Release `383806967` 有四项资产，重复草稿 `383806968` 只有安装器与 `latest.yml`。只删除不完整的 `383806968`，随后复验 git tag 仍是 annotated tag object `f51c2a9…`。
- 正式发布 Release `383806967`，标题 `Mineradio v2.0.0：视觉预设切换不再回退`，`published_at` `2026-09-07T03:11:12Z`，非 draft / 非 prerelease；`releases/latest` 已复验指向 `v2.0.0`。
- 四项资产全部回下载到临时目录复算：

| 资产 | 字节 | SHA256 |
| --- | ---: | --- |
| `Mineradio-oirge-2.0.0-Setup.exe` | 102617004 | `44f22bbcf1b1eef716ddb62bdec5dc6a675413c3670b12eb5d0f0114b402448f` |
| `Mineradio-oirge-2.0.0-Setup.exe.blockmap` | 106805 | `41f79c259d2076658e8d4a748c966a87475ecf29e996bda0f18c2fe70c3a6547` |
| `Mineradio-oirge-2.0.0-SHA256SUMS.txt` | 285 | `c60f1d4d83ffbd7977b80d51209a4f53e094660ecdeeeb9ee27171877fd1a317` |
| `latest.yml` | 359 | `95f664717f1ac1101199d9f56ebd1810a3d07a83351fd8ea3442602936d7cbc6` |

- 三路校验全部通过：GitHub API 的四个 `digest` 与本机 SHA256 逐字相同；SHA256 清单内安装器 / blockmap / `latest.yml` 三条全部匹配；`latest.yml` 的 `version` / `path` 为 `2.0.0` / `Mineradio-oirge-2.0.0-Setup.exe`，`files[0].size` `102617004` 与安装器字节数相同，文件项与顶层 SHA512 都是 `tZl2DLkMwhFPLipgh2vmu6E7JG6phn/xaKq7GzYx1fUDw9zjr3uQnWu8E9fhjBulKj1OKoo0QzpzCYGzY3hIAA==`，与流式复算一致；`releaseDate` `2026-09-07T03:08:32.753Z`。
- `SHA256SUMS.txt` 的 CRLF 遗留仍在：285 字节、3 个 `\r`、无 BOM。这个格式问题不影响 Windows 更新与 GitHub API digest，本轮不顺手改工作流。
- GitHub Actions 给 `actions/checkout@v4` / `actions/setup-node@v4` 留了一条 Node.js 20 deprecated、强制转 Node.js 24 的 annotation，构建仍成功；后续可独立升级 action major，不和本次视觉修复混在一起。
- 资产记录分支 `codex/docs-release-assets-v200`（本条记录自己就在这个分支上，PR 与合并提交号下一版补记）。
...
- **用户的一句话：「让我这个本地播放器和原项目的播放器可以同时使用互不影响」**，两条决定是用户拍的：命名走「Mineradio 二创」；新安装器检测到老身份就提示并卸载。承诺过「应用内界面文案和视觉不动、数据目录不动、曲库设置不丢」，这一版全部守住。
- **根因是安装身份逐字节撞车。** 拉下原项目 `XxHuberrr/Mineradio` 的 `package.json` 核过：他们现在是 `v2.1.0`，`appId` `com.mineradio.desktop`、`executableName` `Mineradio`、`shortcutName` `Mineradio`、`artifactName` `Mineradio-${version}-Setup.${ext}` —— 和本仓库改之前一模一样。同一个 `appId` 推出同一个 NSIS 卸载 GUID `9733721a-009e-52bc-b705-49059cd80258`，Windows 眼里就是一个软件。运行期隔离早由 `desktop/instance-isolation.js` 做完（userData `%APPDATA%\Mineradio-oirge`、单实例锁、AppUserModelId、HTTP 端口、WE 临时目录、更新目录全按 profile 分开），所以只动安装身份：`appId` `com.mineradio.desktop.oirge`、`productName` / `nsis.shortcutName` `Mineradio 二创`、`win.executableName` `Mineradio-oirge`、安装包 `Mineradio-oirge-${version}-Setup.exe`、默认目录 `D:\Mineradio-oirge`。`desktop/main.js` 的 `APP_NAME` + `PRIMARY_PROFILE_ID` 是数据地址，一个字没动；显示身份另开 `APP_DISPLAY_NAME`。
- **旧安装检测的门禁改了两次，第二次是被原项目的仓库逼的。** 第一版按「`DisplayVersion` 以 `1.` 开头 + 目录里有 `.mineradio-install-root`」判断。随后把原项目的 `build/installer.nsh` 和 Release 列表也拉下来核：**原项目写同一个安装标记，而且发过 `v1.1.1`**，两道门都拦不住。唯一可靠的区分是 electron-builder 按 `publish` 配置写进 `resources\app-update.yml` 的发布源 —— 本机 `D:\Mineradio\resources\app-update.yml` 第一行就是 `owner: oirge`，原项目那份是 `owner: XxHuberrr`。现在 `MineradioLegacyInstallIsOurs` 逐行读这个文件比对 `owner: oirge`，版本号不再当门禁。
- **安装根目录在 `HKCU\Software\<GUID>\InstallLocation`**（electron-builder 的 `initMultiUser` 写的，实测 `D:\Mineradio`），卸载记录 `Uninstall\<GUID>` 键下的 `InstallLocation` 才是空的；先读前者，再退到 `DisplayIcon` 反推。
- **重读时揪出一个会让弹窗永远不出现的 bug。** 「新目录是否嵌在旧目录里」原来直接拿 `D:\Mineradio` 当 `$INSTDIR` 的前缀比，默认的 `D:\Mineradio-oirge` 会被判成子目录而静默跳过。两边都补上反斜杠再比（`D:\Mineradio\` 对 `D:\Mineradio-oirge\`）。静默安装里 MessageBox 被 `/SD` 自动作答、`DetailPrint` 也看不见，这种逻辑此前没有任何办法在真机上核对，所以加了探针：环境变量 `MINERADIO_INSTALLER_LEGACY_PROBE=<文件>` 存在时，每一道门的结论追加写入该文件（`legacy=absent / no-marker / not-ours / nested / prompt / declined / uninstall`），不设就什么都不写。
- **叶子名是 `Mineradio` 的目录一律改到旁边**（`MineradioNormalizeInstallDir`：`D:\Mineradio` → `D:\Mineradio-oirge`）。旧版 1.9.3 的卸载器是 electron-builder 默认的 `RMDir /r $INSTDIR`，嵌进去会被连带删掉；原项目的卸载器虽然是逐文件删，但也没理由住进去。没有 D 盘时 `MineradioUsePreferredInstallDir` 也会把 electron-builder 的默认目录归一化到专用叶子名 —— 它的默认叶子名 `APP_FILENAME` 目前恰好等于 `productFilename`（源码 `getWindowsInstallationDirName`），专用名靠这个巧合才对得上，现在不再依赖它。
- **升级路径核过：** 应用内更新器 `desktop/update-installer-launcher.js` 启动安装器只带 `--updated`、不带 `/S` 和 `/D=`，所以 1.9.3 客户端更新时会看到完整安装界面（新默认目录 `D:\Mineradio-oirge`）和那个弹窗；`server.js` 的 `pickReleaseAsset` 按 `.exe` 后缀选资产、`compareVersions` 按数值逐段比，`1.10.0 > 1.9.3` 成立，新安装包名也认得出来。原项目的安装器走 electron-builder 默认的按镜像名结束 `Mineradio.exe`，碰不到 `Mineradio-oirge.exe`；我们的按 `Mineradio-oirge.exe` + 安装目录 + 会话精确筛，也碰不到原项目。
- **`build/after-pack.js` 改按 `platformSpecificBuildOptions.executableName` 找 exe**（逐个 `existsSync` 兜底）。本地打包实测 electron-builder 的 `productFilename` 其实也跟着 `executableName` 走（卸载器叫 `Uninstall Mineradio-oirge.exe`），所以这只是不再依赖它的内部约定，不是修故障；`FileDescription` / `ProductName` 跟 `productName`，任务管理器里和原项目分得开。新身份在本机生成的卸载 GUID 是 `423fa7af-3a0d-514c-9fef-ebee5546ae66`，和旧的 `9733721a-…` 不同 —— 这是实测观察值，代码里没有也不该硬编码它。谱面缓存 `D:\MineradioCache\beatmaps` 故意仍与原项目共用，`.tmp` 名带 `process.pid`。桌面快捷方式名对齐 `nsis.shortcutName`；登录项显式 `name: APP_USER_MODEL_ID`，新增 `reconcileStartupEntryPath()` 把指向旧路径的登录项改到新位置。
- **本地打包踩坑：** 这台机器没有 Visual Studio，electron-builder 默认的 `npmRebuild` 会对 `uiohook-napi` 跑 node-gyp 直接失败；它自带 `prebuilds/win32-x64`，本地打包加 `-c.npmRebuild=false` 即可，CI 上有编译器不受影响，仓库配置没改。`npm ci` 装完 Electron 二进制没下下来，要单独 `node node_modules/electron/install.js`（走 `ELECTRON_GET_USE_PROXY=1` + `GLOBAL_AGENT_HTTPS_PROXY`）。
...
- **起因是用户的一句话加两张截图：「这个是黑的还是没修好啊」 —— 预设 8 选中，整个背景纯黑。** 这不是 v1.9.1 那个惨白没修好，**是另一个缺陷**：v1.9.1 修的是配色（在浏览器里验的），全黑发生在更前面一步 —— 壁纸那一页根本没被允许加载。
- **根因：主窗口导航守卫拦掉了预设 8 自己的 iframe。** `desktop/main.js` 的 `installMainWindowNavigationGuard` 在 `will-frame-navigate` 上写着「一切子 frame 导航一律拦截」，而预设 8 的整幅画面就跑在 `<iframe src="vendor/sonic-workshop/mineradio-bridge.html">` 里（`public/sonic-workshop-preset.js:25` 的 `BRIDGE_SRC`）。于是这个 iframe 永远停在 `about:blank`，`#sonic-workshop-layer iframe{background:#000}` 的黑底铺满窗口，选这个预设时封面粒子整层又是收起的 —— 就是用户看到的纯黑。守卫是 `cd1a75a`（v1.5.2）加的、`f8d624e` 加固过，比预设 8（v1.8.9）早四个版本，**所以这个预设在打包客户端里一次都没显示过**，之前几版对它的调色改动在客户端里全看不出效果。
- **同时修掉守卫读参数的签名错。** Electron 派发 `will-frame-navigate` 的实测形状是 `(event, url, isSameDocument, isMainFrame, processId, routingId)`，而**第一个参数既是事件也是详情**（自带 `url` / `isMainFrame` / `isSameDocument` / `frame` / `initiator` / `preventDefault`）；老代码按 `(event, details)` 把**第二个**参数当详情对象读，那里其实是 url 字符串，于是 `frame` 恒为 `null`、`details.isMainFrame === true` 恒为假，一路走到最后一行 `event.preventDefault()` —— **无论目标是哪儿都拦**，连主 frame 指向本地 `/index.html` 也拦（只是应用是 SPA、初始 `loadURL` 不触发导航事件，所以主 frame 那一半的错拦一直没人撞上）。旧测试是按「详情对象在第二个参数」这个**不存在的签名**调的，所以它一直是绿的 —— 假签名的测试比没有测试更坏。
- **改法：两个 handler 合并成一份 `guard`，参数先过 `readFrameNavigationDetails()` 归一化，再交 `isAllowedFrameNavigation()` 判定。** 归一化认三种形态（新式事件对象自带字段、位置参数 `(url, isSameDocument, isMainFrame)`、旧式详情对象），`frame.parent` 在时优先用它判主/子 frame。判定规则：主 frame 只能停当前 `127.0.0.1` 服务的 `/` 或 `/index.html`（`isTrustedMainDocumentUrl`，原样不动）；子 frame 只放行同端口的 `/vendor/sonic-workshop/mineradio-bridge.html`（新增 `isTrustedWallpaperFrameUrl`，路径全等比较，`?query` / `#hash` 不影响、末尾多个 `/` 就不算），且必须是主 frame 的**直接**子级（`frame.parent.parent` 在就拦）；其余一概拦。整个 handler 包在 `try/catch` 里，读 `WebFrameMain` 抛异常（frame 已销毁）时按拦截处理。
- **放行只是「让这一页加载出来」，没有放开任何权限。** `isTrustedMainFrameSender` 仍然一见 `frame.parent` 就返回 `false`，所有高权限 IPC 都过它，桥接页碰不到主进程能力面；vendor 那份产物不联网、不写本地存储、不含 `eval` 的结论仍由 v1.8.9 起的回归测试钉住。全仓库也没有任何 CSP、没有 `webRequest` 拦截、没改 `webSecurity`，所以守卫是**唯一**那道门。
- **迷你播放器的守卫有同样的签名错，这一版故意没动。** `installMiniPlayerNavigationGuard`（`desktop/main.js:3756`）也按 `(event, details)` 读，效果是「什么都拦」；迷你播放器里没有任何 iframe，它自己的页面走 `loadURL`（不触发导航事件），所以错拦不产生任何可见后果，方向上也是偏安全的那一侧。改它要连带改 `tests/mini-player-security-settings.test.js` 的接缝注入，与本次修复无关，留着。
...
## v1.2.60 桌面歌词滚轮缩放与更小字号
- `v1.2.60` 在桌面歌词解锁态加入滚轮缩放：鼠标命中实际歌词或悬停工具条时，上滚放大、下滚缩小，每档 `0.05`；锁定态、拖动中和透明空白区域不响应。
- 缩放范围从 `0.72–1.55` 扩展为 `0.20–1.55`，最小视觉字号约从 `42px` 降至 `12px`；`− / +` 按钮、DIY 滑块、覆盖层和主进程统一使用新边界，超长歌词可继续自适应至约 `8px`。
- 解锁工具条在 `− / +` 之间显示实时字号百分比；歌词拖动、悬停和中键判断余量随实际字号缩放，最小档每侧约 `3px × 2px`，固定工具条不并入主进程歌词热区。
- 字号仍由主 renderer 持久化；旧歌词窗口、禁用/锁定态和 ignored 请求继续被所有权门禁或回滚。鼠标穿透及既有歌词视觉基线不变。
- 全套 `node --test` 143/143、51 个 JS 文件语法、真实 Chrome DOM/滚轮交互和 `git diff --check` 均绿。
- 本次发布上传安装器、blockmap、`latest.yml` 和 `1.2.59 -> 1.2.60` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.60`，正文使用本节正式更新说明。

## v1.2.59 桌面歌词缩放与精准拖动热区
- `v1.2.59` 在桌面歌词解锁工具条增加紧凑 `− / +` 字号按钮，继续复用并持久化现有 `desktopLyricsSize`，调整范围保持 `0.72–1.55`。
- 拖动、悬停捕获和中键锁定热区改为贴合动画中的实际歌词矩形并裁剪到 viewport，只保留最多 `16px × 10px` 余量；滚动变换先更新、热区后采样，工具条固定贴在歌词上方且不能启动拖动。
- 覆盖层字号 IPC 保持主 renderer 为设置真源，并校验当前歌词 renderer、启用态和解锁态；旧窗口或 ignored 请求会回滚。锁定穿透、轮询所有权、歌词颜色/描边/发光基线不变。
- 全套测试 139/139、`node --check`、前端内联脚本解析、Playwright DOM 交互和 `git diff --check` 均绿。
- 本次发布上传安装器、blockmap、`latest.yml` 和 `1.2.58 -> 1.2.59` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.59`，正文已改为本节正式更新说明，不再使用占位文案。

## v1.2.58 FLAC Enhanced LRC 逐字歌词适配
- `v1.2.58` 识别 FLAC 内嵌 Enhanced LRC 的 `<mm:ss.xx>` 逐字时间标签，不再把标签当作歌词正文显示，并复用现有 `words` 结构提供逐字高亮。
- 同时间字词、行尾结束标签、多行时间戳、普通 LRC 与双语合并均有回归覆盖；用户问题样本实际导入得到 88 行歌词、837 个逐字高亮组，正文标签残留为 0。
- 本次发布上传安装器、blockmap、`latest.yml` 和 `1.2.57 -> 1.2.58` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.58`。
...
- 新增回归测试覆盖截断回退与未截断保持增量两条路径；全套 102/102 、`node --check`、AST 门禁、`git diff --check` 均绿。
- 本次发布上传安装器相关资产：安装包、blockmap、`latest.yml` 和 `1.2.45 -> 1.2.46` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.46`。

## v1.2.45 本地文件代理越权读取修复
- `v1.2.45` 修复本地文件 HTTP 代理 `/api/local-file` 越权读取：代理此前只校验随机令牌，未强制目标位于已授权曲库根目录内，持令牌的本机请求可读授权目录以外任意文件（含 `..` 穿越）。
- 代理改为与 IPC 通道一致的授权模型：主进程注入 `resolveAuthorizedLocalFile`，代理读取前强制目标落在已登记曲库根目录内；未注入授权函数时缺省拒绝（Fail-Closed）。
- 新增端到端回归测试（缺省拒绝、授权内放行、授权外拒绝、`..` 穿越拒绝、令牌错误拒绝）；坑点沉淀于 `.context/pitfalls/mineradio-local-file-proxy-authorization.md` 并在 `AGENTS.md` 建索引。
- 本次发布上传安装器相关资产：安装包、blockmap、`latest.yml` 和 `1.2.44 -> 1.2.45` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.45`。

## v1.2.44 本地资产与桌面状态内存优化
- `v1.2.44` 继续围绕本地大曲库、长时间播放和桌面覆盖层驻留做内部内存优化，不改变 UI、布局、文案、玻璃质感、电影视觉或播放入口。
- 本地歌词原文采用精确当前对象播放租约；切歌、清空队列、迟到文件读取和缓存水合不会让旧歌曲副本重新持有长文本，释放后的 `ready` 索引摘要仍可按单曲恢复。
- 空曲库和切库后台资产任务使用令牌与队列所有权隔离；本地封面、Object URL、内嵌 Blob、范围读取字节和歌单/节奏缓存均按消费者生命周期裁剪。
- 桌面歌词、壁纸、迷你播放器和中键轮询增加窗口/进程所有权释放门禁；禁用或主窗口恢复后不保留重型载荷，锁屏期间不安排恢复任务。
- 本次发布只上传完整安装器、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.43 -> 1.2.44` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.44 本地资产与桌面状态内存优化版`。
...
- 镜像候选继续受摘要校验保护，无摘要镜像不能绕过 `ensureMirrorCanBeVerified()`。
- 完整安装包与快速补丁统一使用 12 秒响应头超时和 30 秒正文空闲 watchdog；换线重置状态，成功、异常和镜像切换均清理计时器。
- 完整包异步落盘会处理部分写入、写盘失败和响应超量；本地磁盘/权限错误不再重复下载到其他镜像；快速补丁在整组校验/备份后应用，中途失败恢复原文件并清理临时文件，应用阶段错误不再切换镜像重试。
- 完整安装包边下边累计摘要，落盘后走 `verifyStreamedUpdatePayload()` 流式校验，缓存复用路径仍用 `verifyUpdateFile()`；校验读缓冲复用 1MB 块，`closeUpdateFileHandle()` 保留主错误。
- MediaPipe 手势帧复用掌心 scratch 和固定 tips 数组，掌心只计算一次并传给张开度与骨架绘制；捏合、握拳、旋转、HUD 和 Canvas 输出保持不变。
- 本次发布继续只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.40 -> 1.2.41` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.41 更新候选链路与手势帧低分配优化版`。

## v1.2.40 桌面歌词与更新链路低分配/抗卡死优化
- `v1.2.40` 继续降低桌面歌词和封面取色交互中的短命对象分配，不改变 UI、视觉质感、歌词输出或播放入口。
- 封面取色放大镜在打开时缓存一次 canvas JPEG/CSS 背景，`mousemove` 只更新坐标；关闭时释放 canvas 与 Base64 引用。
- `normalizeDesktopLyricText()` 按原文和 single/double 行数模式缓存最终字符串，持续高帧率同步不重复创建清洗数组。
- 快速补丁首包保持 12 秒超时，正文读取使用 30 秒空闲 watchdog，并在成功、异常和镜像切换路径清理计时器。
- 更新错误分类会归一化 `AbortError` / `TimeoutError`，并检查 Node `fetch` 的嵌套原因，避免把连接拒绝误报为 DNS 故障。
- 本次发布继续只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.39 -> 1.2.40` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.40 桌面歌词与更新链路低分配/抗卡死优化版`。

## v1.2.39 桌面歌词覆盖层载荷低分配优化
- `v1.2.39` 继续降低桌面歌词/壁纸覆盖层高帧率同步的短命对象分配，不改变 UI、视觉质感、歌词输出、播放入口或 3D 歌单架交互。
- 复用歌词快照、动效/播放子载荷、颜色对象、完整桌面歌词 payload 和 wallpaper payload；`beatMap` 首次发送、强制刷新、签名判重和无地图清理语义保持原样，IPC 快照取得后释放节奏图与壁纸封面引用。
- 复用对象只在当前同步调用内读取，preload 通过 `ipcRenderer.invoke` 立即序列化 payload；代码旁明确标注不能跨帧保存引用。
- 6 类关键状态及随机交错状态的旧/新载荷和签名逐项一致，确认对象身份复用且重复发送不会残留旧 `beatMap`。
- 本次发布继续只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.38 -> 1.2.39` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.39 桌面歌词覆盖层载荷低分配优化版`。
...
- 分析帧向 `updateCinemaTrackProfile()` 传入复用 sample，线性混合改为模块级 `mixToward()`；曲目分析 profile 使用固定常量对象，不再每帧/每次命中新建。
- 普通/DJ 模式各跑 720 帧节拍模型并覆盖 20+ 次命中，对象复用稳定；电影 profile 720 帧新旧路径状态完全一致。
- 本次发布继续只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.34 -> 1.2.35` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.35 实时节拍与电影画像低分配性能优化版`。

## v1.2.34 歌词光粒与更新缓存并发性能优化
- `v1.2.34` 继续降低高刷屏舞台歌词、安魂视觉和软件更新缓存复用路径的无效工作，不改变 UI、视觉质感、歌词输出、播放入口或 3D 歌单架交互。
- 歌词光粒真正不可见后不再每帧重算 132 个粒子的 396 个 Float32 坐标或请求位置缓冲上传；旋转仍连续累积，淡出与重新开启首帧按当前绝对时间完整重算。
- 主循环移除永久隐藏小封面的 transform 写入和空 Float 层调用；Home 波形复用已查询节点，音频包络函数不再在分析帧内重复创建。
- 安魂呼吸偏移与歌词镜头边界复用同步 scratch 对象；相机姿态只更新位置和朝向时，不再紧接主相机更新后重复重算投影矩阵。
- 缓存安装包校验按文件、版本、大小和摘要执行 singleflight；16 路本地并发下载请求验证为一次文件打开/哈希、一个 ready 任务，失效和异常路径结束后会清理在途记录。
- 随机状态模型覆盖光粒初始关闭、开启、淡出、跨不可见阈值和长时间关闭后重开；所有可见帧 396 个坐标及旋转逐值一致。
- 本次发布继续只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.33 -> 1.2.34` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.34 歌词光粒与更新缓存并发性能优化版`。

## v1.2.33 渲染空闲与更新校验性能优化
- `v1.2.33` 降低高刷屏空闲渲染、舞台歌词持续动画和完整安装包校验的 CPU、GPU 上传与内存抖动，不改变 UI、视觉质感、歌词输出、播放入口或 3D 歌单架交互。
- 涟漪纹理改为触发时标脏、活动期间同步、最后一帧清零；5,000,000 个纯空闲合成帧中，上传请求从 `5,000,000` 次降为 `1` 次，旧/新混合触发序列逐帧数据一致。
- 舞台歌词复用三组详情 profile、预解析 glow 基色和帧级 spark 颜色；星河两组 uniform 颜色只在调色板变化时重算，保留原颜色亮度下限和材质 copy 语义。
- 完整安装包改为异步固定缓冲分块单遍哈希；Electron 42 对实际 `v1.2.32` 约 `99.87MiB` 安装包三轮基准中，中位耗时约下降 `15%`，事件循环最大延迟约下降 `98%`，额外 RSS 约下降 `98%`。
- 校验链路覆盖 SHA256、SHA512 Base64/Hex、双摘要、仅大小、摘要/大小不符、文件缺失、缓存复用和镜像回退；下载任务只有校验成功后才进入 ready。
- 本次发布继续只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.32 -> 1.2.33` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.33 渲染空闲与更新校验性能优化版`。

## v1.2.32 歌词游标与曲库排序性能优化
- `v1.2.32` 优化播放中的歌词当前行定位和本地大曲库目录排序，不改变 UI、视觉质感、播放入口或 3D 歌单架交互。
- 舞台歌词顺序播放时只从上次游标向前推进；切歌、跳播和歌词数组变化时使用 upper-bound 二分回退，保留原 `50ms` 提前量与同时间戳语义。
- 300 行、180 秒、144Hz 合成播放验证中，歌词时间戳读取从 `2,372,940` 次降到 `52,026` 次，约减少 `45.6` 倍；`72,840` 次随机与边界定位结果和旧算法完全一致。
- 本地曲库文件名排序复用 `Intl.Collator('zh-Hans-CN', { numeric: true, sensitivity: 'base' })`；20,000 项合成基准从约 `1,680.9ms` 降到 `59.9ms`，排序结果逐项一致。
- 同步修正更新面板遗留的 `APP_VERSION = 1.2.27`，前端、服务端与构建版本统一为 `1.2.32`。
- 本次发布继续只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.31 -> 1.2.32` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.32 歌词游标与曲库排序性能优化版`。

## v1.2.31 播放定时器按状态休眠优化
- `v1.2.31` 将播放进度/听歌统计从永久 `280ms setInterval` 改为按真实播放状态运行的单任务自调度，不改变 UI、视觉质感、播放入口或 3D 歌单架交互。
- 暂停、结束、空队列和普通隐藏状态没有播放 tick；播放开始与恢复可见时立即重启。
- 连续 `play` / `playing` 事件不会叠加定时器；暂停、结束和空源会清理唯一任务。
- “直播后台保持”开启时隐藏播放继续使用原 `280ms` 频率，听歌统计和进度更新语义保持不变。
- 本次发布继续只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.30 -> 1.2.31` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.31 播放定时器按状态休眠优化版`。

## v1.2.30 长音频时长与进度刷新低抖动优化
- `v1.2.30` 修复长本地歌曲、长录音和有声内容的时长单位误判，并继续降低播放进度与 Windows Media Session 的常驻刷新开销；不改变 UI、视觉质感、播放入口或 3D 歌单架交互。
- 本地 `song.duration` 按秒处理，`durationMs` / `dt` 按毫秒处理；无效或无限时长会继续回退到有效字段。
- 进度文本只在整数秒或总时长变化时重新格式化，进度条仍按原精度平滑更新；节点重建后会强制补写当前进度和时间。
- Media Session 的 900ms 节流提前到时长、位置和播放速率读取之前，命中节流时直接返回。
- 本次发布继续只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.29 -> 1.2.30` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.30 长音频时长与进度刷新低抖动优化版`。

## v1.2.29 歌词绘字与进度拖动低抖动优化
- `v1.2.29` 继续围绕歌词贴图绘制、进度拖动和粒子清理做低风险性能与稳定性优化，不改变 UI、视觉质感、播放入口或 3D 歌单架交互。
- 歌词字距测量/绘制使用 Unicode code point 游标，保留 Emoji、扩展汉字和原字距输出，不再创建 `Array.from` 字符数组。
- 进度拖动只读取一次轨道布局并绑定发起指针；忽略右键和其他指针，零宽轨道、切歌竞态及捕获丢失时会安全恢复状态。
- 每轮三个拖动粒子改为单次 DOM 插入和单个清理任务，粒子数量、位置、动画时长和质感保持不变。
- 本次发布继续只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.28 -> 1.2.29` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.29 歌词绘字与进度拖动低抖动优化版`。

## v1.2.28 列表签名与进度热路径优化
- `v1.2.28` 继续围绕列表签名、本地音质缓存键、播放进度时间格式化和引导尾迹裁剪做低风险性能优化，不改变 UI、视觉质感、播放入口或 3D 歌单架交互。
- 歌曲副标题/本地音质缓存键、3D 歌单架 draw/rebuild 签名、队列渲染指纹、本地曲库面板签名和本地快照/记录签名改为直接字符串拼接。
- 播放进度时间格式化去掉热路径 `padStart`；引导尾迹长度与过期点改为批量 `splice` 清理。
- 本次发布继续只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.27 -> 1.2.28` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.28 列表签名与进度热路径优化版`。

## v1.2.27 双尺寸迷你播放器
- `v1.2.27` 在保留原 `360 × 84` 标准迷你播放器的基础上，新增独立 `268 × 58` 极简迷你播放器。
- 极简版不创建封面节点、不加载封面，保留歌曲名、歌手、上一首、播放/暂停、下一首和返回主界面。
- 设置面板与托盘菜单都可切换“标准（带封面）/ 极简（无封面）”，选择由主进程持久化。
- 标准版和极简版分别保存拖动位置；极简窗口的状态 IPC 不携带封面字段。
- 本次发布只上传安装器相关资产：安装包、blockmap、`latest.yml`、SHA256 校验文件和 `1.2.26 -> 1.2.27` 快速补丁；Portable ZIP 跳过。
- Release 标题使用 `Mineradio v1.2.27 双尺寸迷你播放器版`。
