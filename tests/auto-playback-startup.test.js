...
test('自动播放开关常驻 DIY 高级页并接在启动恢复链路上', () => {
  ...
  assert.match(
    appSource,
    /initAutoPlaybackControls\(\);\s*(?:initReplayGainControls\(\);\s*)?(?:initAudioChainControls\(\);\s*)?(?:initGaplessControls\(\);\s*)?if \(LOCAL_ONLY_MODE\) scheduleSavedLocalMusicFolderRestore\(700\);/,
  );
