...
function testFrontendVersionMatchesPackage() {
  ...
  assert.equal(lock.version, pkg.version, 'package-lock.json 顶层版本必须与 package.json 一致');
  assert.equal(lock.packages && lock.packages[''] && lock.packages[''].version, pkg.version, 'package-lock.json 根包版本必须与 package.json 一致');
