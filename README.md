...
### 最新版本 v1.8.3 (2026-09-05)

- **修掉黑屏与卡顿的进程层根因。** 这两个症状其实是同一个根因的两种表现：播放器过去启动时无条件强行越过 Chromium 自己的显卡屏蔽名单，而那份名单里躺着的正是会把画面渲染成全黑的驱动组合；GPU 进程崩了退回软件渲染、再崩再退，这个反复过程就是肉眼看到的持续卡顿
- **新增三档显卡阶梯，会自动往下降：** 标准档（和过去**逐字节相同**，正常机器上的表现一点没变）→ 兼容档（不再对显卡做任何指定，尊重 Chromium 的屏蔽名单）→ 软件渲染档。**连续两次 GPU 崩溃才降一档**，单次崩溃多半只是驱动瞬时抽风，为此重启一次应用比黑屏还烦人
- **升级或换显卡驱动之后会自动退回标准档重试一次**，不会把你永久钉在软件渲染上 —— 那本身就是卡顿
- **黑屏时的手动逃生门：`MINERADIO_GPU_MODE=default|compatible|software`。** 命令行里 `set MINERADIO_GPU_MODE=software` 再启动就能先把界面救出来。它优先级最高、**不写入配置**，而且用它点定档位之后自动降级完全让路
- **主窗口终于有了绘制失败兜底。** 过去只有迷你播放器接了渲染进程崩溃与加载失败，主窗口一崩就只剩一片全黑、永远不会自己回来。现在加载失败、渲染进程崩溃、首次加载抛错都会自动重试：**同一时刻只重试一次**，间隔逐次拉长，**最多 3 次就停手只留日志** —— 无限重载一个加载不起来的页面只会把「黑屏」换成「黑屏 + 满负载」
- 重试用的是页面重载而不是重建窗口，**所以窗口位置和置顶状态不会被打乱**
- **补上启动兜底：窗口 6 秒还没显示出来就强制显示一次。** 无边框透明窗口的「可以显示了」这个事件在 Windows 上并不保证触发，而窗口是隐藏着建出来的 —— 事件不来就等于「双击图标什么都没发生」
- **从睡眠、锁屏回来或者插拔显示器之后，主窗口会强制重画一次。** 透明窗口的画面表面在这些时刻可能已经失效，而重画比重载便宜得多，**不会打断正在播的歌**
- 渲染进程卡住时只记日志，**不顺手重载** —— 那会把播放状态丢掉，这个取舍是刻意的
- **界面一行未动**，也没有为档位加任何设置项：盯着黑屏的人点不到设置面板，所以恢复必须全自动
- 全量 Node 回归 `913/913` 通过（新增 42 例）
- 已知边界：降档只在真的看到 GPU 进程崩溃时触发。**有些驱动会把画面渲染成全黑但 GPU 进程一次都不崩**，那种情况仍然要靠 `MINERADIO_GPU_MODE` 手动降档

<details>
<summary>v1.8.2 多格式歌词：KRC / QRC / TTML 一并认</summary>
