...
- 标准迷你播放器窗口是固定 `360 × 84` 的透明窗口，`body { padding: 6px }` 后外壳只有 `348 × 72`，收回态 `54 × 54` 封面上下仅剩约 15px、左右仅剩约 7px 余量。直接把封面缩放或光晕系数调大会被操作系统窗口裁掉半圈光晕，属于“错位”类视觉缺陷；放大上限必须锁在几何余量内，想加强只能加强曲线前段而不是抬高上限。
- 封面律动强度和封面光晕强度是两个独立档位。光晕不能挂在 `pulseEnabled` 上，否则关掉律动就连呼吸光也一起消失；两个开关关闭时也必须保留用户已选强度值，重新打开要恢复原档位而不是回落到默认 1.00。
...
- `runMiniPlayerPulseTimer()` 在隐藏播放态优先读取 `beatAnalyser`，按低频均值、低频峰值和前 96 桶能量计算原始强度，再用 `miniPlayerPulseBaseline` 提取短期峰谷；后台切换或 `AudioContext` 恢复瞬间如果低平滑分析器返回空帧，必须回退到已接入输出链路的 `analyser`，不能把有效音频覆盖为零。`document.hidden` 与 `desktop-window-state` 任一隐藏信号都要触发定时器同步，`visibilitychange` 监听必须调用 `syncMiniPlayerPulseTimer()`；禁用或停止播放时同时衰减样本与基线。回归测试：`tests/mini-player-pulse.test.js`。
- 迷你封面律动/光晕的强度换算全部放在 renderer 的 `applyState()`：`signal = Math.pow(pulse, 0.72)`，再按 `saturateMiniEffect(signal * strength)`（即 `1 - Math.exp(-1.15 * level)`）压成 `0~1` 的 `--mini-pulse` / `--mini-glow`。CSS 系数就是几何上限，不再承担强度语义：收回态封面 `scale(calc(1 + var(--mini-pulse) * 0.195))`、展开态 `0.125`，光晕最大到 `0 0 calc(6px + var(--mini-glow) * 14px)`。想让效果更明显只能调 `strength`，不得直接抬高这些系数。`box-shadow` 一律只读 `--mini-glow`，缩放一律只读 `--mini-pulse`，两者不得交叉引用。回归测试：`tests/mini-player-visual.test.js`。
- 强度档位 `miniPlayerPulseStrength` / `miniPlayerGlowStrength` 取值 `0 ~ MINI_PLAYER_EFFECT_STRENGTH_MAX(3)`，`1.00` 是标准力度、`0` 等于关闭；主进程 `normalizeMiniPlayerVisual()` 用同一个 `MAX_MINI_PLAYER_EFFECT_STRENGTH` 夹紧，renderer 侧 `readMiniEffectStrength()` 兜底非法值。饱和曲线保证 `strength = 3` 时驱动值仍 `< 1`，封面留在 `360 × 84` 窗口内（实测最坏边距 1.7px）。回归测试：`tests/mini-player-visual.test.js`、`tests/mini-player-state-cache.test.js`。
...
- 相关实现：`public/index.html`、`public/app.js`、`public/mini-player.html`、`desktop/main.js`、`desktop/desktop-overlay-state-cache.js`、`desktop/mini-player-recovery-session.js`、`desktop/mini-player-state-cache.js`、`server.js`、`scripts/test-mini-player-memory.ps1`、`tests/desktop-lyrics-ipc-ownership.test.js`、`tests/desktop-lyrics-mouse-poller.test.js`、`tests/desktop-overlay-state-cache.test.js`、`tests/desktop-overlay-disabled-ipc.test.js`、`tests/desktop-overlay-main-update.test.js`、`tests/desktop-overlay-window-ownership.test.js`、`tests/local-asset-cache-ownership.test.js`、`tests/local-beat-cache-residency.test.js`、`tests/local-cover-thumb-promise-ownership.test.js`、`tests/local-file-range-memory.test.js`、`tests/local-library-persistent-memory.test.js`、`tests/local-library-background-refresh.test.js`、`tests/local-library-background-cancellation.test.js`、`tests/local-library-incremental-truncation.test.js`、`tests/local-lyric-cache-residency.test.js`、`tests/local-media-object-url-residency.test.js`、`tests/playlist-cover-cache-residency.test.js`、`tests/mini-player-main-gates.test.js`、`tests/mini-player-pulse.test.js`、`tests/mini-player-recovery-session.test.js`、`tests/mini-player-state-cache.test.js`、`tests/mini-player-visual.test.js`
