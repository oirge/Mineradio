...
`v1.7.0` 引入，`v1.7.4` 收窄成只有一种插件：主题（`theme`）。音源与歌单能力（含插件的全部网络与播放通道）在 `v1.7.4` 整体移除。安装包自带两份声明式主题（默认不启用），其余全部由用户自己导入。
...
- `tests/plugin-theme-reach.test.js`：主题覆盖面与 `--th-*` 从 renderer 到迷你窗口的整条转发链路。
