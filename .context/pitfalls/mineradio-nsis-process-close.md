...
- 一台机器可能同时运行多个不同安装目录的播放器进程，二创版 `Mineradio-oirge.exe` 还会和原项目 `XxHuberrr/Mineradio` 的 `Mineradio.exe` 并存。全局按进程名结束会误杀其它实例，也会误杀原项目正在放歌的进程。

## Solution / Convention

- 应用内启动更新安装器时使用独立子进程；确认安装器创建成功后设置 `appQuitting = true` 并调用 `app.quit()`，避免关闭到托盘拦截退出。
- `initMultiUser` 从注册表恢复现有安装目录后，`MineradioUsePreferredInstallDir` 必须在该目录存在 `.mineradio-install-root` 时保留 `$INSTDIR`，不得再次回落到 `D:\Mineradio-oirge`，否则自定义目录覆盖安装会被迁移到错误位置。
- NSIS 自定义进程检查必须只筛选：
  - 进程名为 `${MINERADIO_PROCESS_EXE_NAME}`（二创版为 `Mineradio-oirge.exe`，不要写死 `Mineradio.exe`）；
...
- 覆盖安装 E2E 必须使用独立 `appId` 和安装目录。通过标准为：安装器退出码为 `0`、目标目录进程与监听端口消失、安装文件已覆盖、正式 `D:\Mineradio-oirge` 主进程 PID 和路径保持不变。不要比较全部 Chromium 子进程 PID。
