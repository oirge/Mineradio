...
- 收回态窗口保持 `360 × 84`，穿透规则自 v1.7.12 起改为「指针在窗口内 = 整窗可交互，离开窗口 = 恢复穿透」：转发/真实 `mousemove` 只会在指针位于窗口上时到达，收到即视为在窗（`pointerInsideWindow`），整窗保持命中以便任意位置右键弹菜单、双击恢复；`document mouseleave` 清零在窗与热区标记立即恢复穿透。旧规则「只有封面热区参与命中、空白区永远交还桌面」已废弃——那是右键菜单只能在封面上弹出的根因。封面热区（外扩 `6px`）仍单独负责「悬停展开」的触发与收回排定。
