...
**插件只能换外观。** `v1.7.4` 起插件只有主题一种，音源与歌单能力（以及插件的全部网络与播放通道）已整体移除：插件没有联网能力，拿不到本地文件、账号和其它插件的数据，能做的事就是给界面和默认背景轻量换色，再加一点 CSS。安装包自带 `午夜靛蓝` / `暖琥珀` 两份完整主题，以及 `深海微光` / `暗焰余晖` / `冷杉夜雾` 三份纯背景主题，默认都不启用。
...
纯背景主题可以只写这三个 `vars`，不写 `css`。它仍然是普通 `theme` 插件，因此也受主题互斥规则约束：启用一份纯背景主题会关掉当前完整主题，面板配色随即回落到默认值。
