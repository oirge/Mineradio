...
适用版本：`v1.7.4` 起。插件由用户从「设置面板 → 插件 → 管理 → 安装插件文件」导入。

**插件只能换外观。** `v1.7.4` 起插件只有主题一种，音源与歌单能力（以及插件的全部网络与播放通道）已整体移除：插件没有联网能力，拿不到本地文件、账号和其它插件的数据，能做的事就是给界面换色和加一点 CSS。安装包自带 `午夜靛蓝` / `暖琥珀` 两份主题，默认不启用。
...
```json
{
  ...
  "author": "yourname",
  "theme": {
    "vars": { "--th-panel-bg": "rgba(10,10,14,0.86)", "--th-hairline": "rgba(255,255,255,.08)" },
