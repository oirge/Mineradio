...
- 当前源码检查点：`v1.2.91` 重发候选；桌面歌词手动 bounds 保存、恢复和定位会保留仍可操作的部分越界真实坐标，避免 `y=-171` 被夹成 `y=0`，纵向位置偏好清理旧 bounds 与既有优化继续保留。
- 当前工作分支：`codex/complete-v1.2.79`。
- 最近正式安装包 Release 基线：现有 `v1.2.91` 正在按同版本覆盖重发；新候选安装器、blockmap、`latest.yml` 和本地 Portable ZIP 已生成，GitHub 资产待替换。
