...
- 当前源码检查点：`v1.2.87`；在 `v1.2.86` 已发布修复基础上，继续优化桌面歌词 30/60 FPS renderer 的文本归一化和 Unicode 绘字短命分配，并补齐完整 HTML 内联脚本语法门禁。
