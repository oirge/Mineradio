...
- 当前源码续版：`v2.0.3`（**SSA 歌词与发布链路加固，已发布**。`.ssa` 与 `.ass` 共用动态 `Format:` 解析和字幕优先级，兼容 SSA v4 的 `Marked` 首列；发布构建加 `--publish never`，显式创建或复用唯一同 tag 草稿 Release，重跑用 `--clobber`，多个同 tag Release 时安全失败；Actions 升 v5；SHA256 清单改为 LF、无 BOM UTF-8。回归 `1076/1076`。tag `v2.0.3` = tag object `ac634a30…` 指向 `30b3ba5`，Release `386043356` 已设 Latest。）
...
- **未发布修复（工作区已改、未提交、未发版；用户要求「只有有实际功能更新我说了之后才能发布新版本」）：更新公告解析漏显。** 症状：v2.0.6 的更新弹窗只显示「输出设备」一条，音量弹层那条不见了。根因不是公告写错，而是 `server.js` 的 `normalizeReleaseNotes`（`server.js:678`）有一条 `if (text.length > 96) continue;`，**会静默丢弃超过 96 字的整行**；那条 140 字的说明被吞掉，位置被后面的内部备注填满。修法二合一：① 列表行（`- ` / `* ` / `1. `）放宽到 `RELEASE_NOTE_BULLET_LIMIT = 600`（只作超长防护），非列表散段落仍走 `RELEASE_NOTE_LINE_LIMIT = 96`；② 新增 `isHeading = /^#{1,6}\s/`，在 `isHeading || sectionAction` 时跳过，所以「说明」这类不在 `releaseSectionAction` 白名单里的标题也不当内容。两个常量声明在 `releaseLineEncodingPenalty` 之后、`normalizeReleaseNotes` 之前（这段夹在 `tests/update-release-notes.test.js` 的 `releaseLineEncodingPenalty` → `pickReleaseAsset` 切片里，只依赖 `{Buffer}`，别再引外部模块）。`tests/update-release-notes.test.js` 从 4 例扩到 8 例（已验证 `1111/1111`）。**发布时要连同下次真实功能更新一起发，不要为它单独开版本。** 线上 v2.0.6 的公告正文已顺手收敛为每条 ≤96 字（双保险）。
...
- 更早续版：`v1.9.0`（**两个「音域回响」对齐原项目**：用户报「两个音域回响视觉预设效果和原项目的不太一样」，对着上游 `XxHuberrr/Mineradio` commit `89c0d23` 逐项核出五处差异并全部改成上游行为。**① 预设 7 以前吃不到细粒度音频**——上游读 `public/js/modules/03-beat/06-sonic-audio-monitor.js`，本仓库只有 5 个粗粒度标量、八段是反推的；那层已整个移植成 **`public/sonic-audio-monitor.js`**（IIFE 挂 `window.MineradioSonicAudio`，导出 `{BAND_EDGES, BASE_BINS, BEAT_WINDOWS, reset, settings, snapshot, step}`），上游常量一个不改：八段 `32–58 / 58–118 / 118–260 / 260–720 / 720–1800 / 1800–4200 / 4200–9000 / 9000–16000` Hz、六个鼓点窗口 `Deep/Club/Kick/Punch/Body/Wide`（自动跟踪 + 自适应噪声底）、`BASE_BINS 512`、默认 `{sensitivity 100, bandStart 1, bandEnd 4, threshold 32, pulseStrength 62}`；**边界是赫兹不是 bin 下标，`sampleRate`/`fftSize` 由调用方注入**。接线三条缺一不可：`index.html` 里 `<script src="sonic-audio-monitor.js">` **必须排在两个预设脚本和 `app.js` 之前**、`sonicAudioMonitorModule()` 用 `typeof` 守卫取模块、`sonicTopographyCtx.audio = sonicAudioFrame || sonicTopographyCoarseAudio`（**只有监视器缺席或 `fx.sonicAudioMonitorEnabled === false` 才用那个粗粒度壳**）；停播按 `0.08^dt` 衰减。**② `currentTime` 往回跳超过 `0.30s` 清瞬态并把 `prev` 清零，往前跳不清**；拖完那一帧读成一次满频段起拍**是上游行为、不是 bug**。**③ 去掉 `beatPulse * 1.35`**（v1.8.8 的入口放大，涟漪比原项目早就是它），`doesNotMatch(/beatPulse \* 1\.35/)` 钉住。**④ 地形配色不能和歌词配色共用一份**：本仓库 `lyricTextPaletteFromHsl` 是更早一代（亮封面改深青字、饱和夹 `0.42~0.78`），地形跟着走必然暗一档灰一档，而**直接把歌词那份换成上游公式是红线**；解法是同一次封面扫描并行算第二份 `lyricGroundPaletteFromHsl()`（内部 `lyricHighImpactTextHsl(hsl, {avgL, minS: 0.90})`），**只**写进 `groundPrimary`/`groundSecondary`/`groundHighlight`，预设 7 只读这三个（`palette.groundPrimary || palette.primary` 兜底），**歌词那五个字段一个字节没动、以后也别「顺手统一」**。地形支兜底照上游七条 + 暗色调红紫那条。**一处历史怪癖别当 bug 改**：文字支的第三个入参传 `Math.max(0, best.score)`（打分值当彩度用）、地形支传真彩度，所以灰封面下歌词是偏红的暖灰、地形退银蓝，两边确切输出都被测试钉住。**⑤ 预设 8 配色源换成原始封面取色**：新增 `rawPrimary/rawWarm/rawCool/rawLight/rawDark/rawAccent/rawAverage` 与按占地面积取色的 `rawArea*`（`24` 一档分桶、桶内点数即权重、六角色按 `avoid`/`minDistance` 挑、`colors` 上限 10），`workshopCustomThemeForRegions` 按 `rawArea*` → `raw*` → 歌词色三级取；**壁纸层不再读 `visualTintColor`**。**⑥ 删掉 `QUALITY_GRID_SIZE`、`gridSize` 固定回原作 `320`、五档一样**（原作的地形密度/涟漪半径/流星尺度都是照 320 调的）。**因此 v1.8.9 那条「本机适配只有三处」基本作废**，只剩「地形基色自压暗」与 `showPlayerController: false`。测试技巧：`tests/lyric-cover-palette-split.test.js` 用**跳注释与三种引号的大括号配平切片器**从 `app.js` 抠 17 个调色函数进一个 `vm.runInNewContext`（**别为了测调色板整份读 `app.js`**）；跨 realm `deepStrictEqual` 要先 `JSON.parse(JSON.stringify(v))` 拷回本 realm；监视器内部有 `followValue` 平滑，**单帧 band 值的精确断言必然是错的**。全量回归 `987/987`（基线 `965`，新增 12 + 9 例），三条负向守卫 `QUALITY_GRID_SIZE` / `visualTint` / `beatPulse * 1.35` 防本机适配长回来。**已知边界：结论全来自逐项比对上游源码加测试，没有在真实窗口里逐帧对着原项目比过观感。** 逐条见本文件 `### 2026-09-05 - 两个音域回响预设对齐原项目（v1.9.0）` 小节。）
- 更早续版：`v1.8.9`（视觉预设「音域回响」现在是两条实现并排：**预设 7 = Ajin 的 three.js 重写（`SONIC_TOPOGRAPHY_PRESET_INDEX = 7`，细节见下一条 v1.8.8）、预设 8 = CmzYa 的 Wallpaper Engine 原作（`SONIC_WORKSHOP_PRESET_INDEX = 8`）**，同一件作品的两条路，别当重复功能删掉一个。**预设 8 不是移植、是 vendor 一份第三方构建产物**：`public/vendor/sonic-workshop/` 下四个文件（`mineradio-bridge.html` 13,909 B、`project.json` 8,615 B、`assets/index-Z-j1MQ-r.js` 1,262,837 B、`assets/index-Bhwp8mwk.css` 9,754 B，合计约 1.3 MB）逐字节复制自上游 `XxHuberrr/Mineradio` commit `89c0d23`、一个字节都没改（发版前用 `sha256sum` 逐个核过——**这类「未修改任何字节」的断言必须先核再写**），版权属 CmzYa（Workshop 物品号 `3747222633`），整幅画面由这份产物在 iframe 里自己渲染。**上游的 NOTICE 既没写这份产物的出处、也没提 Ajin 的名字，两条都由本仓库补记在 `NOTICE.md` 的 `## Community Contributions`（已拆成「预设 7」「预设 8」两块，改的时候别把两条实现的署名串了）。** vendor 之前做过安全审计并把结论钉成永久回归测试：12 个记号（`XMLHttpRequest` / `WebSocket` / `EventSource` / `sendBeacon` / `importScripts` / `eval(` / `new Function` / `localStorage` / `sessionStorage` / `indexedDB` / `document.cookie` / `navigator.geolocation`）零命中、外链主机只有 6 个文档域名，**将来重新 vendor 一个会联网的包是 CI 失败、不是静默发货**。宿主侧只有 `public/sonic-workshop-preset.js`（845 行，IIFE 挂 `window.MineradioSonicWorkshop`，导出 `{INDEX, isActive, update, clear, pushProperties, onPresetChange}`）：桥接页把 WE 宿主 API 8 个函数整套 shim 掉并暴露 `__mineradioApplyAudio` / `__mineradioApplyMedia` / `__mineradioApplyProperties`，**模块优先直调这三个、`contentWindow` 上没有时才退回 `frame.postMessage({type,…}, '*')`，两条路都要保**。**`#sonic-workshop-layer` 必须 `insertBefore` 到 `#canvas-container` 之前的同一个父节点**（同处 z-index 0/1 那层，作为最后一个 `z-index:0` 兄弟压在 `#wallpaper-board` / `#custom-bg` / `#album-bg` / `#theme-bg-tint` 之上、粒子画布之下）；**整层与层内每一个后代都是 `pointer-events:none !important`**，另加 `inert` / `aria-hidden` / `tabIndex=-1` / `draggable=false`，`body.sonic-workshop-active` 给上层界面让路、深度睡眠隐藏列表也带上它——**这几条一条都不能为了「让壁纸能交互」去掉**，播放器全部 UI 都在它上面。预设 8 下 `particles.visible = !skullPresetActive && !workshopPresetActive`（`bloomParticles` / `floatGroup` / `backCoverGroup` 同步）把封面粒子整层收起，`skullBackdropDim` 取 `0.82` 与预设 7 同档，`isSoftFlowPreset` 现覆盖 5 / 7 / 8。推送节奏与音频整形系数全部保持上游原值：音频 `33ms`、媒体 `250ms`、属性 `1000ms`，各自按内容键去重，**所以换歌或改配色是当场推一次、不等窗口到点**；淡入 `7.5` / 淡出 `5.0`，透明度 `≤0.01` 时整层摘出 DOM；整形 `BODY_GAIN 0.33` / `PEAK_GAIN 0.12` / `GAMMA 1.55` / `MIN_FLOOR 0.035` / `LOW_LIFT 0.035`（只作用于 `i<36` 的低频段）/ 暂停 `×0.12` / 输入增益默认 82（`/100`，夹 40~100），**`TARGET_MAX_SAMPLE 0.52` 用归一化输入其实永远不会触发（可达上界约 0.457），它是留给非归一化来源的护栏、不是当前生效的夹取点**。本机适配只有三处、都写在模块文件头（**① 与 ③ 已在 v1.9.0 改掉，见当前源码续版那条**），别当移植错漏「修回去」：① 配色取 `stageLyrics.coverPalette` 的 primary / secondary / highlight（冷色 / 暖色 / 涟漪与峰值），认不出退回 `#62d6ff` / `#ff8f4a` / `#33e6ff`；② 地形基色自己压暗（`mixHex('#05070c', primary, 0.16)`），本仓库的调色板是给歌词用的、没有暗部区域色；③ `gridSize` 跟画质档位联动 `{eco:224, balanced:288, high:320, ultra:384}`，**默认档 high = 320 = 原作 `project.json` 的默认值，开箱即与原作一致**；`showPlayerController` 保持 `false`（播放器 UI 是本仓库自己的）、`showAlbumCover` `true`。**预设数量是数据驱动的，这一版没改任何数字**（夹取用 `presetMeta.length - 1`，`public/app.js:6149` / `:35036`）：`presetMeta[8] = 原作 CmzYa`（`nameHtml` 后缀 `Wallpaper Engine`）、`presetDisplayOrder = [0, 7, 8, 6, 5, 4, 2, 1, 3]`、`presetIcons` 必须与预设数等长。打包零改动（`build.files` 本来就是 `public/**/*`）；`<script src="sonic-workshop-preset.js"></script>` **必须排在 `app.js` 之前**，`app.js` 用 `typeof MineradioSonicWorkshop !== 'undefined'` 取模块，且**不应出现 `sonic-workshop-layer` 或 `mineradio-bridge.html` 这两个字符串**（DOM 与路径只归模块管，测试用 `doesNotMatch` 钉住），帧循环用模块外预分配的 `sonicWorkshopCtx` 逐帧填字段、不要写对象字面量。`scheduleWorkshopThemeTransition` 在这份实现里**实际是死的**（`applyWorkshopThemeTransition` 总会清掉 `state.themeTransition`，跨主题交叉淡入由产物自己做），别以为漏了调用。`tests/sonic-workshop-preset.test.js` 26 例用 `vm` + 手写假 DOM **真跑一遍 `update()`**，钉的是行为（插入位置、点不穿、淡入到摘除的整个生命周期、三档节流在假时钟上 10ms→无 / +40ms→音频 / +300ms→媒体 / +800ms→属性、整形后的确切采样值、调色板角色映射、`project.json` 交叉核对 `gridSize.value === 320` 与 `workshopid`）；app.js / CSS / index.html 那部分跑不起来，另有一层源码文本断言，两层加起来才是完整守卫。加预设会打红 `tests/sonic-topography-preset.test.js` 四处（预设名单 9 条、图标数、`skullBackdropDim`、`isSoftFlowPreset`），属于预期，**改断言、别改实现去迁就旧断言**。全量回归 `965/965`（v1.8.8 基线 `939`，新增 26 例）；**壁纸层的实际观感没有在真实窗口里逐帧核对过**，观感项留给反馈再调。逐条见 `docs/PROJECT_MEMORY.md` 的 `### 2026-09-05 - 音域回响补上 Wallpaper Engine 原作（v1.8.9）` 小节，发布过程见 `RELEASE.md` 顶部小节。）
- 更早续版：`v1.8.8`（① 视觉预设「音域回响」= 预设 7，常量名 `SONIC_TOPOGRAPHY_PRESET_INDEX = 7`。**这是移植，不是原创**：地形算法逐段移植自上游社区分支 `XxHuberrr/Mineradio` 的 `public/sonic-topography-preset.js`（GPL-3.0，与本仓库同许可，commit `89c0d23`），它又标注移植自 `yin-yizhen/sonic-topography` 1.1.1（commit `3ff303e`），原始创意是 CmzYa 的 Wallpaper Engine 作品《音域回响》（Workshop 物品号 `3747222633`），出处链写在 `NOTICE.md` 与 `public/sonic-topography-preset.js` 的文件头。**v1.8.7 发出去的那一版是我在自己 shader 框架里做的频谱环、和原项目不是一回事，用户当场纠正「和原项目的回响不一样这是原项目 https://github.com/XxHuberrr/Mineradio」——「原项目」指上游这个 fork、不是 Workshop 作品本体，以后别再把它读成「源码拿不到、只能做原创致敬」。** 那一版的 `SPECTRUM_ECHO_*` 子系统（64×64 `DataTexture` / `spectrumEchoTex` / `uSpectrumTex` / 着色器 `uPreset > 6.5` 两处分支 / `updateSpectrumEchoField` / `resetSpectrumEchoField`）已整体删除、`grep` 零命中，`else if (uPreset < 6.5)` 放宽回 `else`，`presetMeta[7]` 的说明从 `频谱回响 · 致敬 CmzYa` 改成 `音域地形 · 移植 CmzYa`，`tests/spectrum-echo-preset.test.js` 已 `git rm`。**这批地形改动已作为 v1.8.8 发出去了**（用户先说「改好先不发布新版本我本地先看一下效果」，看完之后改口「更新好后发布新版」）：五处版本钉一起动到 `1.8.8`。**另外 v1.8.7 的 GitHub Release 已经不在了**（tag `v1.8.7` 仍在远端，但 `releases/tags/v1.8.7` 返回 404、草稿 0 个，`Build and Release` run `33938522336` 当时是成功的，说明是发布之后被删掉的），所以 `releases/latest` 从 `v1.8.6` 直接跳到 `v1.8.8`，别拿 v1.8.7 当在线的对照版本。另一条路本来就通——项目已有 Wallpaper Engine 集成（`desktop/wallpaper-engine-library.js`，`WALLPAPER_ENGINE_APP_ID = '431960'`），装了原作就能直接当桌面壁纸层或播放器背景板跑。**移植落在独立兄弟脚本 `public/sonic-topography-preset.js`（1083 行，IIFE 挂 `window.MineradioSonicTopography`，导出 `{INDEX, isActive, update, clear, onPresetChange, pointerRipple}`），不塞进 `app.js`** —— 这样地形用自己的 `ShaderMaterial`，`app.js` 的着色器字符串机器一个字节都不用碰（`bloomVs` 由 `vs` 两次逐字节精确替换派生，锚点 `'uniform float uMouseActive, uPixel, uColorMixT, uLoading;'` 与 `'gl_PointSize = sz * uPixel * uPointScale;'` 必须保持原样）。`public/index.html` 里 `<script src="sonic-topography-preset.js"></script>` **必须排在 `app.js` 之前**（`app.js` 顶层就会取 `window.MineradioSonicTopography`）；打包不用动，`build.files` 本来就是 `public/**/*`。渲染结构是四个 `InstancedMesh`（地形 `BoxGeometry(boxWidth,1,boxWidth)` × gridSize²、悬浮方块 80、流星 20、尾迹 200，全部 `frustumCulled = false`、每帧只写实例矩阵），精度按画质档位封顶 `QUALITY_GRID_CAP = {eco:112, balanced:160, high:192, ultra:224}`，默认密度 46 → 156×156，`spacing = TERRAIN_BASE_SIZE(168) / gridSize`、柱宽 `spacing * (0.9/1.05)`。**音频量纲是唯一需要适配的地方**（**这条已在 v1.9.0 作废**，八段监视器已移植成 `public/sonic-audio-monitor.js`、`1.35` 也去掉了，见当前源码续版那条）：上游有细粒度频谱监听，本项目只有 `{bass, mid, treble, beat, energy}` 五个标量，所以 `readMineradioAudio` 从这五个值推 8 段频谱与 `kickEnvelope`；上游阈值不动（涟漪 `kickEnvelope > 0.58`、0.32 复位、流星 `> 0.62 && random < 0.045`），改的是入口——本项目 `beatPulse` 峰值约 `0.62~0.92`，所以帧循环传 `beat: Math.min(1, beatPulse * 1.35)`。**帧钩子必须排在 `updateSkullParticleLayer(dt, frameShelfState);` 之后**（也就是 `particles.rotation.x/y +=` 已经加完的位置），地形才和主粒子层共用同一帧旋转，与上游顺序一致；传入的 `sonicTopographyCtx` 在模块外预分配、逐帧填字段（`scene` 1739 行 / `orbit` 1951 行 / `particles` 4812 行都声明在它之后，所以声明处只能是空壳），60fps 下不造垃圾；模块从不读 `ctx.screenHeight` / `ctx.dpr`，别往 ctx 里加没人用的字段。涟漪 uniform 打包沿用上游 `vec4(x, z, start, ±strength)`，**w 取负号 = 白色细涟漪**（军鼓与高频），正号是底鼓蓝涟漪；`RIPPLE_MAX 10` / `RIPPLE_LIFETIME 4.8` / 软淡出 `2.1` 秒是**插值进 GLSL 的**（着色器里能看到 `2.10,4.80`），改常量等于改着色器；`syncRippleUniforms` 排在 `updateAudioTriggers` 之前，所以第 N 帧新增的涟漪要到第 N+1 帧才进 uniform（测试钉住了这条）。`update` 的顺序也被钉住：透明度积分 → `if (!active && opacity < 0.01) { 隐藏; return; }` → `if (!scene) return;` → `ensureLayer` → 旋转 → 布局 → `sonicTime += dt * (0.45 + motionSpeed*0.017)` → uniforms → 触发器 → 写矩阵 → `root.visible = opacity > 0.02`；**透明度积分在 `!scene` 早退之前**，所以「没有场景也会淡出」是有意行为、不是漏判（写测试时踩过：无场景那一帧把透明度积到 0.05，下一帧切到预设 5 就不再满足 `!active && opacity < 0.01`，于是照样建层——这是正确的淡出行为，要断言「冷启动不分配」必须换一个全新 harness）。画布单击（不是拖动）打涟漪挂在 `mouseup` 上，判 `!mouseDownAt.hadDrag && !isPointerOverUi(e)`，按压时长换强度（`0.25 + 秒数*2.6`，上限 `3.0`），屏幕坐标映射到世界 ±17。背景星河在这个预设下压到 `0.82`（`skullBackdropDim`，与上游一致，不压会糊成一片）；`isSoftFlowPreset` 仍覆盖 5 与 7；`setPreset` 给 `p === 7` 的相机基线是 radius `8.4` / phi `0.18`（v1.8.7 那版是 `8.6` / `0.16`）。上游有四个字段在本项目里是死的、移植时故意丢掉了：`manualYaw` / `dummyObj` / `lastOrbitTheta` / `orbitThetaReady`。**三个行为等价的写法差异**：涟漪寿命常量插值进 GLSL、`bindVisualRotation` 只吃 ctx、`syncTerrainUniforms` 把 `themeAudio` 提成一个共享局部量。② **全屏退出黑屏卡顿**根因是遮罩被后到的信号一路顺延（退出时 resize、边界还原、`leave-full-screen` 状态推送连发好几轮，每轮都重排回亮计时器），现在 `scheduleFullscreenTransitionReveal` 只允许提前——`if (revealDue && due >= revealDue) return;`，resize 或尺寸已跳变用 50ms、纯状态推送用 110ms；硬上限 `FULLSCREEN_TRANSITION_MAX_COVER_MS = 320` **独占 `deadlineTimer` 一个槽**，resize/state 只能重排 `revealTimer`、抢不掉它，动作前摇 110ms、收尾 220ms、降低动效偏好 20/90/30/80；`desktop/main.js` 的 `applyWindowedBounds` 边界已到位就不再 `setBounds` 但仍 `sendWindowState`，`leave-full-screen` 与 `leave-html-full-screen` 都立刻推状态；CSS **窗口壳过渡不能带 `filter`**（`#desktop-window-shell` 承载 WebGL 画布，加 filter 会让整窗每帧重新合成），只留 `will-change:transform`。测试 `tests/sonic-topography-preset.test.js` 16 例（THREE stub 直接读实例矩阵，钉住的是行为：流星未激活时藏在 `y = -1000` 且缩放 0、落地正好撒 10 粒尾迹、尾迹一秒内过期、切走预设整层释放显存、冷启动停在预设 5 时一个实例都不分配；`Math.random` 注入以便确定性；three.js r128 的 `Color` 不做 sRGB→linear，所以 stub 直接按十六进制解析）+ `tests/fullscreen-exit-transition.test.js` 7 例，全量 `939/939`（v1.8.7 基线 `932`，新增 16 例、删掉失效的 9 例）；`node --check` 干净；**但地形层与重调后的全屏过渡都没有在真实窗口里逐帧核对过**，观感项（涟漪强度、配色跟封面的搭配）留给用户反馈再调。（当时留的待定项——上游那份 CmzYa 的 WE 打包产物与桥接页**没有** vendor 进来——已在 v1.8.9 结掉，见上一条。）逐条见 `docs/PROJECT_MEMORY.md` 的 `### 2026-09-05 - 音域回响改成移植上游实现（v1.8.8）` 小节（同日的 `### 2026-09-05 - v1.8.7` 小节只作历史记录，别照它找代码），发布过程见 `RELEASE.md` 的 v1.8.8 小节。）
- 更早续版：`v1.8.6`（左下小封面接上歌曲详情。`public/index.html` 的 `#thumb-cover` 加 `onclick="openTrackDetailModal('song')"` 与 `title="歌曲详情"`，`public/app.css` 的 `#thumb-cover` 规则加 `cursor:pointer`，**一行 JS 都没新增**。空闲态不需要额外守卫：`openTrackDetailModal` 第二行就是 `if (!song) { showToast('先播放或选择一首歌'); return; }`，而且 `#thumb-wrap` 拿到 `.visible` 之前是 `pointer-events:none`。**中途试过让上下两行一致（把 `#thumb-artist` / `#control-artist` 的 `openTrackDetailModal('artist')` 改成 `'song'`）被用户当场否掉——原话「这个不用改了」——已完整还原；歌名进歌曲详情、歌手行进歌手详情这个分工是设计，别再顺手改。** 底部 `#control-cover` 故意没跟着改：它是 `aria-hidden="true"` 的装饰性 `div`，挂点击要连带处理无障碍语义。`#thumb-cover` 里那句 `transition:transform .15s ease` 是历史遗留、当前没有任何 `:hover` 触发它，本轮故意没补 hover 缩放，测试用 `assert.doesNotMatch(appCss, /#thumb-cover:hover/)` 把「不加」也钉住。`tests/now-playing-detail-click.test.js` 4 例（读 `index.html` 属性 + `app.css` 规则 + `app.js` 里那句兜底的正则），全量回归 `916/916`（上一版基线 `912`），四例先在改前源码上对跑过、封面 `onclick` 与 `cursor:pointer` 两条判红；`public/app.css` 只动第 573 行一行；**真实窗口里没有肉眼点过**。发布过程见 `RELEASE.md` 顶部小节。）
...
- Git 仓库以远端 `https://github.com/oirge/Mineradio.git` 为准；本地检出路径随机器变化，旧文档里的 `C:\Users\Administrator\Desktop\Mineradio-main`、`C:\Users\oirg\Desktop\mok\Mineradio-sync`、`E:\桌面\播放器软件\Mineradio\resources\app` 在当前环境都不存在。
...
> **2026-09-11 修正（以此为准）**：用户随后反馈舞台档「没有原项目动态效果」，要求与上游一模一样。复核确认首版只搬了外层卡片——详情页仍用宿主旧动画常数、动态详情只朝向相机而**位置不跟随**、详情每帧被更新两次、镜头进入/离开速度没有消费者、舞台还沿用侧栏的静态/常驻配置。此前的接口测试与 DOM 事件派发不能证明视觉一致。取材基线已核实为本地上游 `89c0d230c3f1f792e5d9639781ebbf724c4efbfe`；通过本机代理核对远端 HEAD `328a087a14e79d5267a3de54d29108c34ed6cf60`，仅晚两个提交且歌单架动态公式无变化（差异只是上游接了内置在线歌单数据，本仓库继续用本地数据）。上游 `stage` 分支不使用 `shelfSummonSlide/Stagger/Scale/Parallax`，这四个无效滑条已从舞台参数区移除，不为了迁就无效控件去改上游舞台公式。发布门禁：**本版已在用户明确要求下发布为 `v2.0.7`**（此前"版本仍 2.0.6"是发布前状态）。
>
> **修正后实际落地（已跑通并真机验证）**：
> - **详情动画吃上游参数**：`makeContentListManager(options)` 只有 `classic: true` 时读 `shelfDetailSettings` 的 `openDuration/closeDuration/rowDuration/intro/parallax`，并补了上游的 **`placeDynamicDetailFromCamera`**（按相机前/右/上三轴构造详情位置，`open()` 与逐帧 `update()` 都调用）；`skullDetail` 在 classic 下改用 `shouldUseSkullSafeShelfCamera()`。side 分支的常数与行为逐字未动。
> - **详情每帧只更新一次**：删掉引擎 `update` 包装器里那次重复的 `contentList.update`，改由上游 `baseUpdate` 内唯一一次调用。真机统计 `详情update : 引擎update = 50:50`（比例 1）。
> - **镜头速度接入**：`updateCamera` 在 classic 拥有的 shelf focus 生命周期内按 `shelfCameraEnterSpeed/ExitSpeed` 缩放 `focusEase/radiusEase`，退出收敛后清 `orbit.focus.shelfClassic`；只作用于 `shelfClassic===true` 的 focus，不动电影系统其他路径。
> - **舞台独立基础设置**：基础滑条仍编辑 `fx.shelf*`，但侧栏与舞台各存一份（`fx.shelfSideSettings` / `fx.shelfStageSettings`，`fx.shelfSettingsMode` 记录当前档），切档时先停旧引擎再换有效配置。舞台默认取上游 `dynamic + auto`、`size 0.92 / offset -0.34,-0.2,0.12 / angle -11 / bgOpacity 0.79`；侧栏默认仍是 `static + always`（`fxDefaults` 的 `shelfCameraMode/shelfPresence/shelfSize/...` 未改动）。真机验证 A→B→A 往返、各档值互不覆盖、`saveLyricLayout/readSavedLyricLayout` 往返一致。
> - **参数存档可用**：`CLASSIC_SHELF_FX_RANGES` 改成惰性 `classicShelfFxRanges()`，修掉范围表在启动首次 `readSavedLyricLayout()` 时还是 `undefined` 导致舞台参数读不回来的问题（20 条专属参数，重启能恢复）。`shelfSummonSlide/Stagger/Scale/Parallax` 仍保留在存档与 `fxDefaults`（只是不再暴露滑条），避免旧档案丢失。
> - **依赖与生命周期**：宿主补 `saveLyricLayout`、`restoreBottomControlsAfterShelfExit`（清 `controlsShelfSuppressUntil` 后 `revealBottomControls`）、`safeShelfCloseContent`；`dispose()` 立即释放详情并作废异步令牌（不是淡出）；停用旧引擎时清 pin/hover/focus。数据签名 `shelfClassicSignature` 覆盖本地歌单增删/排序与成员内容，`shelfClassicItemAt` 支持队列虚拟数组懒构造。
> - 测试：`tests/shelf-classic-engine.test.js` 25 例（真 THREE 建卡、0.8s 签名重建、每帧一次详情、停用释放与迟到回调、重入停旧实例、多帧位姿、上游常量等价）；全量 Node 回归 **1136/1136**。真机（`127.0.0.1` 静态服务）验证：舞台建卡 2 张、详情 group 相机空间偏移 `(0.543,-0.089,-4.986)` 在两组完全不同的相机位姿下保持不变（即位置真跟拍）、切档零 `window` error、舞台截图见正常大卡 + 详情面板。

<details>
<summary>首版接线记录（部分描述已作废，不作为当前实现或验收依据）</summary>

- 日期：2026-09-11。用户澄清真实需求：**保留现有「侧栏」歌单架不动，另加一个「舞台」档，切换用原项目那套歌单架**（"原项目的歌架单是这样的…我要的是保留我这个原本的侧栏歌架单增加一个切换为舞台歌架单"）。**不是**"现有引擎 vs 原版引擎"两套独立开关。按仓库规矩：只有用户明确说有实际功能更新后才发版，先留工作区。
- **最终模型（与首版实现相反，务必看这段）**：`fx.shelf` 三态 `off / side / stage` 不变，但**引擎由模式决定**——`side` 走本仓库现有 `makeShelfManager()`（`shelfManagerDefault`，未改一字），`stage` 走上游原版引擎（`shelfManagerClassic`，来自 `public/shelf-classic.js`）。切换入口：DIY 视觉控制台「3D / 手势」里原本就有的「关闭 / 侧栏 / 舞台」三态（`#shelf-seg`）+ 主界面底部控制条的 `#shelf-view-btn`（`toggleShelfStageMode()`）。**音量弹层里的歌单架切换已移除**（用户没让放那里）。
- **关键函数**：`shelfEngineIsClassic()` = `shelfModeValue()==='stage' && 有 classic 模块`；`applyShelfEngineForMode(mode)` 按目标模式换 `shelfManager` 单例并 `setMode`；`setShelfMode()` 里调它（这是唯一的模式入口）；`updateShelfClassicParamsVisibility()` 只在 stage 档显示 `#shelf-classic-params`；`updateBottomShelfToggle()/toggleShelfStageMode()` 主界面按钮。**已删除** `fx.shelfEngine` 开关、`normalizeShelfEngine`、`#shelf-engine-seg`、`#main-shelf-engine-seg`、`updateShelfEngineControls/updateMainShelfEngineControls`（首版那套"独立实现开关"整体作废）。
- **保留**：`public/shelf-classic.js` 引擎（约 65 KB，IIFE 挂 `window.MineradioShelfClassic`，由 `AppData\Local\Temp\shelf-build\assemble.js` 从本地上游源码树 `.tmp-upstream-we/public/js/modules/04-shelf/` 拼装）、19 个顶层几何函数分派（classic 档转原版）、`CLASSIC_SHELF_FX_RANGES` + `normalizeClassicShelfFx()`（舞台档 21 条参数，导入/导出/存档共用）、`#shelf-classic-params` 分组与 `syncClassicShelfSliders/initShelfEngineControls`。
- **架构关键**：`shelfManager` 是全局单例，换引擎只重新赋值它，主循环与百余处引用一行不用改。
- 验证：全量 Node 回归 `1118/1118`（`tests/shelf-classic-engine.test.js` 7 例仍钉引擎本身；首版测试里对"独立实现开关"的断言若引用 `#shelf-engine-seg` 需一并清掉）。**浏览器真实验证**：`setShelfMode('stage')` → `shelfManager!==shelfManagerDefault` 且 `getMode()==='stage'`、原版参数组显示、底部按钮态；`setShelfMode('side')` → 回到 `shelfManagerDefault`、参数组隐藏；底部 `toggleShelfStageMode()` 来回切一致；舞台引擎连跑 10 帧（2 张卡）/侧栏 5 帧零报错、零 `window` error。
- 待办：`tests/shelf-classic-engine.test.js` 中有无引用已删除的 `normalizeShelfEngine` 需复查；观感未逐帧核对。

- **上游源码树就在本地**：`.tmp-upstream-we/public/js/modules/04-shelf/`（未纳入 git），7 文件 2710 行。取材不必走网络。
- **根因式判断**：本仓库 `public/app.js` 的那份歌单架就是上游同源分叉魔改版（`makeShelfManager` 等 ~30 个同名符号），所以"原版"不是另写一套，而是把上游 `00-layout-hover.js` + `01-manager-core.js` + 设置层 `shelfDetailSettings/shelfSummonSettings/durationEaseFactor` 逐字节搬成**第二个引擎**，与现有版并存。
- **新文件 `public/shelf-classic.js`**（约 65 KB / 1381 行，IIFE 挂 `window.MineradioShelfClassic`）：由脚本 `AppData\Local\Temp\shelf-build\assemble.js` 从本地上游源码树拼装（读 00+01、去重复 `var`、删模块级自建实例、把 `currentItems()` 的平台登录分支换成 `D.getShelfItems()`），**绝不手抄**。导出 `createClassicShelfEngine(deps)` + 原版几何/热区/悬停函数 + `settings/detailSettings/summonSettings` + `CLASSIC_FX_DEFAULTS`。整段 IIFE **不污染任何全局**。
- **注入的宿主依赖**（`createClassicShelfEngine(deps)`）：渲染 `THREE/scene/camera/renderer/uniforms/particles/pointerParallax`、数据与工具 `songCoverSrc/queueItemKey/compactCount/requestPlaylistCover/readableInkForHex/shelfAccentHex/shelfAccentRgba/shelfSettings/shelfAlwaysVisible/shouldUse.../pulseObjectValue/showToast/playShelfSelectTick/setFocusZone/setPeek/updateEmptyHomeVisibility/loadPlaylistIntoQueueById/playQueueAt/setShelfMode/togglePlaylistPanel/clampRange`，以及回调 `getShelfItems()`（本仓库的"特别喜欢/独立歌单/全部音乐/队列"）、`snapshotLive()`（**每帧重取会被重新赋值的数组/对象**：playQueue、particles、fx、userPlaylists 等，因为 D 是快照，数组换引用后必须重取）、`pushShared()`（把引擎内的 `shelfPinnedOpen/shelfVisibility/shelfOpenAnimAt` 回写顶层变量）。
- **架构关键（决定改动面很小）**：`shelfManager` 是全局单例，主循环 `app.js:44819` 与所有跨子系统都通过它 + 共享的 `shelfPinnedOpen/shelfVisibility/frameShelfState` 工作。所以切换只需 `applyShelfEngineSelection()` 里重新赋值 `shelfManager`，**那 105 处引用与主循环一行不用改**。顶层几何函数（`shelfLayoutProfile/shelfHotZoneWidth/isShelfClickZone/...` 共 19 个）用 `shelfWrapGeometry` 包一层分派：classic 档转调原版，默认档逐字节走原实现。
- **`app.js` 改动**：`fxDefaults.shelfEngine='default'` + `normalizeShelfEngine()`；`CLASSIC_SHELF_FX_RANGES` + `normalizeClassicShelfFx()`（导入/导出/存档共用同一套夹取）；`setShelfEngine/updateShelfEngineControls/updateMainShelfEngineControls/initShelfEngineControls/syncClassicShelfSliders/applyShelfEngineSelection`；持久化四处（`readSavedLyricLayout` 的 `result`、`saveLyricLayout`、`normalizeFxArchiveSnapshot` 的 `snapshot`、`applyFxArchiveSnapshot` 调 `applyShelfEngineSelection`）；`resetFx` 保留 `shelfEngine`；启动链插入 `initShelfEngineControls(); applyShelfEngineSelection();`。**注意：`readSavedLyricLayout` 原本是 `return {` 直接返回字面量，为追加原版参数改成了 `var result = { ... }; ...; return result;`，改完必须补回 `catch { return {}; }` 的收尾括号（首次漏了导致 `Unexpected end of input`）。**
- **`index.html` 改动**：`#fx-stage-fold` 内加 `#shelf-engine-seg`（现有风格/原版风格）+ `#shelf-classic-params`（21 条原版滑条，默认 `display:none`）；主界面音量弹层加 `#main-shelf-engine-seg`（vq-seg）；底部 `<script src="shelf-classic.js">` **必须排在 `app.js` 之前**。`public/app.css` 一行未动（复用 `.fx-seg` / `.vq-seg` / `.fx-slider`）。
- **原版 vs 现有版肉眼最大差异（即"原版风格"在还原什么）**：默认形态（上游=自动隐藏+动态镜头；现有=常驻+静态镜头）、骷髅/安魂预设的侧栏横向构图（上游 0.22/0.46/0.76，现有 0.95/1.70/2.20）、详情页动态镜头跟拍（上游每帧面向相机）、hover/唤出节奏（上游 `shelfSummonSettings` 驱动，现有硬编码 rate）、以及原版多出的 21 条参数滑条。
- **数据边界**：本仓库是纯本地播放器（`LOCAL_ONLY_MODE=true`），原版展示的在线平台歌单/播客在这里是死路径，所以**原版档只还原视觉与交互风格，卡片/详情内容仍用本地数据**（`getShelfItems` 给"特别喜欢/独立歌单/全部音乐/队列"；详情列表继续复用宿主已本地化的 `makeContentListManager`，用 `engine.update` 包一层喂 detail 布局）。
- 测试：新增 `tests/shelf-classic-engine.test.js` 7 例（IIFE 不污染全局、manager 接口同构 24 方法、几何/设置层上游默认值、fx 覆盖夹取、getShelfItems 绑定、守卫初值）；`tests/frame-hot-path.test.js` 6 处切片终点从 `\nshelfManager = makeShelfManager();` 改成 `\nvar shelfManagerDefault = makeShelfManager();`；`tests/playback-rate-sleep-timer.test.js` 启动链断言放宽允许中间插入初始化。全量回归 `1118/1118`。
- **浏览器真实验证过**：`127.0.0.1:3399` 起静态服务，页面里 `setShelfEngine('classic')` → `shelfManager` 换成原版、`fx.shelfEngine='classic'`、分段高亮、原版参数组显示；原版引擎连跑 8 帧零报错、几何取上游值（`detailX 1.28`/`openDuration 0.6`）；`setShelfEngine('default')` 切回、`isDefault===true`；切换期间 `window` error/unhandledrejection 零条。
- **观感未逐帧核对**：Node 测试只钉结构与接口，真实窗口里两档的观感差异（尤其细节节奏）留给用户反馈再调。

> 注意：上文提到的 `AppData\Local\Temp\shelf-build\assemble.js` 临时拼装脚本已删除，仓库内没有这份生成工具；后续维护直接改 `public/shelf-classic.js`。首版实际是 20 条参数（不是 21），并且其中 4 条 `shelfSummonSlide/Stagger/Scale/Parallax` 在舞台档本来就无效，已从界面移除。
...
- 数据形态：新增数组键 `mineradio-local-library-folders-v1` 作为唯一权威；旧单根标量键 `mineradio-local-library-folder-v1` 读不到数组时迁移成单元素列表，并作为"主根镜像"随列表首项同步写入。`savedLocalLibraryFolderPath()` 恒返回首项，所有单值消费方（播放会话、维护探测、空库检测）零改动。
- 导入追加：`openLocalFolderImport` 先并入列表再 `reloadLocalLibraryFromAllRoots`；`handleLocalFolderFiles` 的 `persist` 分支逐根 `addLocalLibraryFolderPath`；恢复（`persist:false`）不动设置。
- 合并曲库：`handleLocalFolderFiles` 接受 `opts.rootGroups`，逐根 hydrate + 建索引 + 盖入库时间后合并；不带 `rootGroups` 时退回单根旧路径，单根行为逐字节不变。索引按根保存（绝不把整库写进一个根）。
- 恢复逐根：`loadSingleLocalLibraryRoot` 按 SQLite 交接件 → 旧快照刷新 → 全量扫描取回一个根；取不到的根从设置摘掉、其余照常，全取不到才算失败。
- 监控按根：渲染层把整个根列表交给数组 IPC（`localLibraryWatchRoots`）；变更只重扫那一根（待扫根攒列表），`applyLocalLibraryAutoSync` 只在 `roots.length > 1` 时按 `localLibraryRootForSong` 挑出本根的歌做差异，其余根原地保留后按根顺序拼回。
- 其余：备份导出全部根、导入逐根解析（取消即整体放弃）；播放会话记住歌曲所属根；设置面板新增 `fx-library-fold`（`#library-folder-list` 运行时生成、移除走事件委托）；新键在 app.js / preload / main.js 三处注册。
- 验证：新增 `tests/local-library-multi-root.test.js` 12 例；同步修 `tests/gapless-crossfade.test.js`（折叠区强制展开清单）与 `tests/local-library-categories.test.js`（分组盖章形状）被影响的断言；全量 Node 回归 `1088/1088` 通过；相关 JavaScript 语法检查通过。**真实窗口里没有肉眼验证过「多目录导入 / 移除 / 单目录改动只同步那一根」这条观感链路**，结论来自 `node:vm` 跑真实源码 + 源码断言。

## v2.0.3 SSA 歌词与发布链路加固

- 日期：2026-09-10。发布版本从 `2.0.2` 提升为 `2.0.3`，五处版本钉一起动；已正式发布并设为 Latest。
- SSA 歌词：本地扫描、两处文件导入、同名歌词候选与 MIME 映射均识别 `.ssa`；与 ASS 共用按 `Format:` 动态定位字段的解析器及标准化来源 `ass`，兼容 SSA v4 的首列 `Marked`。
- 发布链路：electron-builder 使用 `--publish never`；工作流显式创建或复用唯一同 tag 草稿 Release，多个同 tag Release 时安全失败；同 tag 任务串行，重跑上传用 `--clobber`；`GH_TOKEN` 仅交给 Release 管理步骤。
- 工作流与清单：`actions/checkout` / `actions/setup-node` 升到 v5；SHA256 清单写为 LF 行尾、无 BOM UTF-8，`sha256sum -c` 可直接跑（CRLF 遗留作废）。
- 验证：SSA、发布工作流及版本一致性定向回归 `25/25`；全量 Node 回归 `1076/1076`；相关 JavaScript 语法检查通过。
- 发布：分支 `codex/release-v2.0.3`，提交 `30b3ba5` / `5bae4cf`，PR #73（merge `d576231`）+ #74（merge `44b39d5`）；annotated tag object `ac634a30…` 指向 `30b3ba5`。**第一次 `Build and Release`（run `34442721960`）失败在「Upload release assets」**：`Generate SHA256 checksums` 里 `cd dist` 后用裸相对名写清单，.NET 工作目录与 PowerShell 位置不一致导致清单没落进 `dist`；`5bae4cf` 改用 `Join-Path (Get-Location)` 后 run `34444159148` 成功。Release `386043356`，`published_at` `2026-09-10T06:22:41Z`，四项资产齐全，Latest=`v2.0.3`；**没有出现双草稿**（`--publish never` + 显式 Release 管理）。四资产回下载三路校验通过（安装器 `102617620` / `3f3527dc…`、blockmap `106795` / `c232dc21…`、清单 `282` / `90c39e20…`、`latest.yml` `359` / `0fb332de…`；`latest.yml` size `102617620`、sha512 `7Eps3b5z…` 与安装器一致，`releaseDate` `2026-09-10T06:14:07.072Z`）。
- 文档回填附带修复：`RELEASE.md` 自 v1.2.61 起被损坏的编码（含换行丢失）已按 `95a36fb` 干净底本修复，`Verify` 增加文档 UTF-8 / 无替换字符门禁，防止复发。

## v2.0.2 全屏切换更顺滑与预设构图修复

- 日期：2026-09-08。用户先后报告全屏进出不流畅、启动后不能播歌、安魂构图与预设 8 落点异常，并要求确认更新日志后发布新版。
- 全屏修复：进入前先铺满目标显示器再 `setFullScreen(true)`；退出时记住原显示器并按它恢复。遮罩期间暂停主渲染、合并 resize，布局 / 玻璃贴图 / 渲染缓冲延后到回亮前；回亮有 320ms 硬上限。
...
- 发布：分支 `codex/release-v2.0.1` 两提交 `11b7581` / `f066344`，PR #69 → merge commit `b9f3441`；annotated tag object `8082f486…` 指向 `f066344`；Build and Release run `34096067323` 成功。双草稿中删掉资产不全的 `383904446`，正式发布 `383904444` 并复验 Latest=`v2.0.1`。安装器 SHA256 `83c2271a0538bba1880190d357b693e9f6dedc035ef01b0990ec098f8331ecec`，四资产哈希、清单与 `latest.yml` 三路校验通过。

## v2.0.0 视觉预设切换与读档恢复一致

- 日期：2026-09-07。用户先报「视觉预设切换有明显异常」，并要求后台测试不影响电脑；确认修复后明确要求「修复好发布新版2.0.0」。
...
- **唯一需要真适配的是音频量纲。**（**这条已在 v1.9.0 作废**：上游那层监视器已整个移植成 `public/sonic-audio-monitor.js`，八段与鼓点包络是真算的、`1.35` 放大也去掉了，见 v1.9.0 那一节；下面这段只作历史记录。） 上游有一套细粒度频谱监听，本项目只有 `{bass, mid, treble, beat, energy}` 五个标量，所以 `readMineradioAudio` 从这五个值推 8 段频谱与 `kickEnvelope`。上游的触发阈值一个都没改（涟漪 `kickEnvelope > 0.58`、0.32 复位再武装、流星 `> 0.62 && random < 0.045`），改的是入口：本项目 `beatPulse` 峰值约 `0.62~0.92`，所以帧循环传 `beat: Math.min(1, beatPulse * 1.35)`。**要调涟漪触发频率就调这个 1.35，不要去动模块里的阈值**，那是上游的手感基准。
...
- 悬而未决：上游仓库里还带着一份 CmzYa 的 WE 打包产物（约 1.26 MB，上游没有附署名文件）与一个 `mineradio-bridge.html` 桥接页。本轮**只搬了原生地形层，没有 vendor 那份产物**。（**已在 v1.8.9 结掉**：产物按字节 vendor 进来落成预设 8，见下一节。）
...
### 2026-09-05 - 音域回响补上 Wallpaper Engine 原作（v1.8.9）

- 用户的指令是「继续更新把我没有的原项目有的视觉预设都移过来」。**逐个比对上游 `XxHuberrr/Mineradio`（commit `89c0d23`）之后，缺口只有一个**：它的 `public/sonic-workshop-preset.js` + `public/vendor/sonic-workshop/`，即 CmzYa 的 Wallpaper Engine 作品《音域回响》本体（Workshop 物品号 `3747222633`）。落成预设 8，常量名 `SONIC_WORKSHOP_PRESET_INDEX = 8`。**预设 7 与 8 是同一件作品的两条实现（7 = Ajin 的 three.js 重写、8 = CmzYa 的 WE 原作），不是重复功能，别当冗余删掉一个。**
- **这一条和预设 7 的性质不同：预设 8 不是移植，是 vendor 一份第三方构建产物。** `public/vendor/sonic-workshop/` 四个文件（`mineradio-bridge.html` 13,909 B、`project.json` 8,615 B、`assets/index-Z-j1MQ-r.js` 1,262,837 B、`assets/index-Bhwp8mwk.css` 9,754 B）**逐字节复制自上游、一个字节都没改**，写「未修改任何一个字节」之前用 `sha256sum` 对着 `/tmp/upstream-mineradio` 的同名文件逐个核过——**这类断言必须先核再写。** 整幅画面由这份产物在 iframe 里自己渲染，本仓库只负责喂数据。
- **署名链在上游是缺的，本仓库补记在 `NOTICE.md`。** 上游的 NOTICE 既没写这份 WE 产物的出处、也没提 Ajin 的名字。本仓库把 `## Community Contributions` 拆成「预设 7（Sonic-Topography，three.js 重写）」与「预设 8（Wallpaper Engine 原作）」两块：7 → Ajin（`yin-yizhen/sonic-topography` 1.1.1，commit `3ff303e`）→ 创意源 CmzYa；8 → CmzYa 产物本体 + 桥接页出处。**改 NOTICE 时别把两条实现的署名串了**，`tests/sonic-workshop-preset.test.js` 有一例专门钉这件事（模块头必须命中 CmzYa / `3747222633` / 上游仓库 / GPL-3.0，且**不能**命中「原创」）。
- **vendor 第三方产物之前做了安全审计，并把结论钉成了永久回归测试。** 1.26 MB 那份 JS 里 12 个记号零命中（`XMLHttpRequest` / `WebSocket` / `EventSource` / `sendBeacon` / `importScripts` / `eval(` / `new Function` / `localStorage` / `sessionStorage` / `indexedDB` / `document.cookie` / `navigator.geolocation`），外链主机只有 `w3.org`(19) / `react.dev`(3) / `tailwindcss.com` / `jcgt.org` / `github.com` / `docs.pmnd.rs` 六个文档域名。**测试里写死了这份 12 记号清单与 6 主机白名单，所以将来重新 vendor 一个会联网的包是 CI 失败、不是静默发货。**
- 宿主侧只有 `public/sonic-workshop-preset.js`（845 行，IIFE 挂 `window.MineradioSonicWorkshop`，导出 `{INDEX, isActive, update, clear, pushProperties, onPresetChange}`）。桥接页把 WE 宿主 API 整套 shim 掉（`wallpaperRegisterAudioListener` / `…MediaPropertiesListener` / `…ThumbnailListener` / `…PlaybackListener` / `…TimelineListener` / `wallpaperMediaIntegration` / `wallpaperReady` / `wallpaperPropertyListener.applyUserProperties`，共 8 个），并暴露 `__mineradioApplyAudio` / `__mineradioApplyMedia` / `__mineradioApplyProperties`。**模块优先直调这三个函数，`contentWindow` 上没有时才退回 `frame.postMessage({type,…}, '*')`——两条路都要保**，测试各走了一遍，postMessage 那条还断言三种消息类型的名字确实出现在桥接页源码里。
- **`#sonic-workshop-layer` 必须插在 `#canvas-container` 之前的同一个父节点里**（`insertBefore`，不是 `appendChild` 到 body）。两者同处 `z-index:0/1` 那一层，作为最后一个 `z-index:0` 兄弟，画面上就压在 `#wallpaper-board` / `#custom-bg` / `#album-bg` / `#theme-bg-tint` 之上、粒子画布之下。**整层与层内每一个后代都是 `pointer-events:none !important`**（`#sonic-workshop-layer,#sonic-workshop-layer *` 那条规则），另加 `inert` / `aria-hidden="true"` / `tabIndex=-1` / `draggable=false`；`body.sonic-workshop-active` 给上层界面让路，深度睡眠列表里也带上这一层。这几条都被测试钉住，**别为了「让壁纸能交互」去掉任何一条**，播放器全部 UI 都在它上面。
- 预设 8 下封面粒子整层收起：`particles.visible = !skullPresetActive && !workshopPresetActive`，`bloomParticles` / `floatGroup` / `backCoverGroup` 同步（和上游一致——壁纸是完成品，叠粒子会脏）。`skullBackdropDim` 在预设 8 取 `0.82`、与预设 7 同一档；`isSoftFlowPreset` 现在覆盖 5、7、8。
- **推送节奏与音频整形系数全部保持上游原值**：音频 `33ms`、媒体 `250ms`、属性 `1000ms`，各自按内容键去重，**所以换歌或改配色是当场推一次、不等窗口到点**（`force` 路径）。淡入 `7.5`、淡出 `5.0`，透明度 `≤0.01` 时整层从 DOM 摘掉。整形：`TARGET_MAX_SAMPLE 0.52` / `BODY_GAIN 0.33` / `PEAK_GAIN 0.12` / `GAMMA 1.55` / `MIN_FLOOR 0.035` / `LOW_LIFT 0.035`（只作用于 `i<36` 的低频段）/ 暂停 `×0.12`，输入增益默认 82（`/100`，夹 40~100）。**核过一遍数量级：全 255 的归一化输入整形后是 `(0.28+0.12)×0.82 = 0.328`，`0.52` 那个上限用归一化输入其实永远不会触发（可达上界约 0.457）——它是留给非归一化来源的护栏，不是当前生效的夹取点。**
- **本机适配只有三处，都写在模块文件头，别当成移植错漏「修回去」**（**其中 ① 与 ③ 已在 v1.9.0 主动改掉——配色换成吃 `rawArea*` / `raw*` 原始封面取色、`gridSize` 固定 320；下面这段只作历史记录**）：① 配色取 `stageLyrics.coverPalette` 的 `primary/secondary/highlight`，角色分工与预设 7 对齐（`uCoolCore`=主色、`uWarmCore`=副色、`uRippleColor`/`uPeakColor`=高光色），认不出退回 `#62d6ff`/`#ff8f4a`/`#33e6ff`；② 地形基色自己压暗（`base = mixHex('#05070c', primary, 0.16)`），不跟封面副色走——本仓库的调色板是给歌词用的、没有暗部区域色；③ `gridSize` 跟画质档位联动 `{eco:224, balanced:288, high:320, ultra:384}`，**默认档 `high` = 320 = 原作 `project.json` 的默认值，开箱即与原作一致**。`showPlayerController` 保持 `false`（播放器 UI 是本仓库自己的）、`showAlbumCover` `true`。
- **预设数量是数据驱动的，这一版没改任何数字**：夹取用 `presetMeta.length - 1`（`public/app.js:6149`、`:35036`），加第 9 条 `presetMeta` 就够。面板文案 `presetMeta[7] = 移植 Ajin`（`nameHtml` 后缀 `Sonic-Topography`）、`presetMeta[8] = 原作 CmzYa`（后缀 `Wallpaper Engine`），`presetDisplayOrder = [0, 7, 8, 6, 5, 4, 2, 1, 3]`，两条并排。`presetIcons` 必须与预设数等长（测试按 `<svg ` 计数）。
- 打包零改动：`build.files` 本来就是 `public/**/*`，vendor 目录自动进包（测试断言了这条）。`public/index.html` 里 `<script src="sonic-workshop-preset.js"></script>` **必须排在 `app.js` 之前**，`app.js` 用 `typeof MineradioSonicWorkshop !== 'undefined'` 取模块；`app.js` 里**不应该**出现 `sonic-workshop-layer` 或 `mineradio-bridge.html` 这两个字符串（DOM 与路径只归模块管），测试用 `assert.doesNotMatch` 钉住。每帧 ctx 仍是模块外预分配的 `sonicWorkshopCtx`、逐帧填字段，不要在帧循环里写对象字面量。
- `scheduleWorkshopThemeTransition` 在本仓库这份实现里**实际是死的**——`applyWorkshopThemeTransition` 总会清掉 `state.themeTransition`，跨主题的交叉淡入由桥接页里那份产物自己做。看到它别以为漏了调用。
- `tests/sonic-workshop-preset.test.js` 26 例：用 `vm` + 一套手写假 DOM（`El` 类：`setAttribute`/`appendChild`/`insertBefore`/`removeChild`/`querySelector`/`style`/`classList`）**真跑一遍 `update()`**，钉的是行为——插入位置、点不穿、淡入到淡出移除的整个生命周期、三档节流（假时钟上 10ms → 无推送、+40ms → 音频、+300ms → 媒体、+800ms → 属性）、整形后的确切采样值（静音恒 `0`、粗粒度 8 段回落 `0.3×0.82 = 0.246`）、调色板角色映射、`project.json` 交叉核对（`gridSize.value === 320`、`workshopid === '3747222633'`）。**app.js / CSS / index.html 那部分接线跑不起来，所以另有一层源码文本断言**，两层加起来才是完整守卫。
- 顺手修了 `tests/sonic-topography-preset.test.js` 里被预设 8 接线打红的 4 条断言（预设名单 9 条、图标数、`skullBackdropDim` 的 `||` 分支、`isSoftFlowPreset` 的 `|| preset === SONIC_WORKSHOP_PRESET_INDEX`），16/16 恢复。**加预设会打红这个文件，属于预期，别去改实现迁就旧断言。**
- 验证：全量回归 `965/965` 通过（v1.8.8 基线 `939`，新增 26 例）。**壁纸层的实际观感没有在真实窗口里逐帧核对过**——「跟着封面换色好不好看」「频谱幅度合不合适」只能靠肉眼，发 v1.8.9 时仍是未确认状态。用户的指令是「更新好后发布新版」「能发布就一起发布」，所以按移植的忠实度发出去、留给反馈再调。（**这一节的「本机适配只有三处」里 ① 与 ③ 已在 v1.9.0 主动改掉**，现状见下一节。）

### 2026-09-05 - 两个音域回响预设对齐原项目（v1.9.0）

- 用户报的是一句话：「两个音域回响视觉预设效果和原项目的不太一样」。v1.8.9 把两条实现都搬齐了，但搬得不够忠实。**重新 clone 上游 `XxHuberrr/Mineradio` commit `89c0d23` 到 `/tmp/upstream-mineradio` 逐项比对**，差异一共五处，全部改成上游行为。**「原项目」= 上游这个 fork，不是 Workshop 作品本体**——这个误读在 v1.8.7 犯过一次，这一轮开头又差点犯第二次。
- **差异 ①（观感差得最明显的一处）：预设 7 一直吃不到细粒度音频。** 上游的地形层读的是 `public/js/modules/03-beat/06-sonic-audio-monitor.js` 产出的八段帧；本仓库是单体 `public/app.js`，没有这一层，只喂了 5 个粗粒度标量（`bass/mid/treble/energy/beat`），八段靠反推、鼓点只有一个笼统脉冲——地形起伏的分布和涟漪的时机因此和原项目对不上。**注意仓库形态的差异：上游是模块化目录（`public/js/modules/**`），本仓库是单体 `app.js` 加同级预设脚本，所以移植的落点是新建兄弟脚本、不是塞进 `app.js`。**
- 新文件 `public/sonic-audio-monitor.js`：`(function (global) { 'use strict'; … })(window)`，挂 `window.MineradioSonicAudio`，导出 `{BAND_EDGES, BASE_BINS, BEAT_WINDOWS, reset, settings, snapshot, step}`。**上游常量一个不改**：八段边界 `subBass 32–58` / `bass 58–118` / `lowMid 118–260` / `mid 260–720` / `highMid 720–1800` / `presence 1800–4200` / `brilliance 4200–9000` / `air 9000–16000` Hz；六个鼓点候选窗口 `Deep 36–82 ×1.04` / `Club 46–118 ×1.22` / `Kick 54–142 ×1.16` / `Punch 68–156 ×1.02` / `Body 86–190 ×0.86` / `Wide 38–155 ×0.78`（自动跟踪挑当前最强的一个，各带自适应噪声底）；`BASE_BINS = 512`；默认设置 `{sensitivity 100, bandStart 1, bandEnd 4, threshold 32, pulseStrength 62}`。**边界是赫兹、不是 bin 下标**，`sampleRate` / `fftSize` 由调用方注入后现算，换 48kHz 或 fftSize 4096 才不会错位。
- 接线三条，缺一不可：`public/index.html` 里 `<script src="sonic-audio-monitor.js"></script>` **必须排在两个预设脚本和 `app.js` 之前**；`app.js` 侧 `sonicAudioMonitorModule()` 用 `typeof MineradioSonicAudio !== 'undefined'` 取模块（插件裁过 `public` 时安静退化，不许让帧循环炸掉）；`sonicTopographyCtx.audio = sonicAudioFrame || sonicTopographyCoarseAudio;`——**只有监视器缺席或 `fx.sonicAudioMonitorEnabled === false` 才退回那个粗粒度壳**，壳子保留是为了这两种退路，不是冗余代码。停播不硬切静音，按上游的 `0.08^dt` 指数衰减收回去。
- **差异 ②：喂给地形层的鼓点被放大了 `1.35` 倍。** v1.8.8 为了对齐本仓库 `beatPulse` 的峰值区间（`0.62~0.92`）在入口乘了 `1.35`，结果涟漪整体比原项目触发得早。上游是原值直给，这一版去掉，`tests/sonic-topography-preset.test.js` 加 `doesNotMatch(/beatPulse \* 1\.35/)` 防它长回来。
- **差异 ③：拖进度 / 换歌不清瞬态。** 上游在 `currentTime` 往回跳超过 `0.30s` 时 `reset()` 并把 `prev` 清零，往前跳不清。**代价是拖完那一帧确实读成一次满频段起拍——这是上游行为，核过源码，不要当 bug「修掉」。**
- **差异 ④：预设 7 的地形配色发暗发灰，根因是调色板代际差。** 上游拿同一份颜色既画歌词又画地形；本仓库的 `lyricTextPaletteFromHsl`（`public/app.js:8403`）是更早一代——亮封面 `avgL > 0.82 && chroma < 0.12` 改深青字（`#064b5b`/`#168c88`/`#315f68`）、`lightText = avgL < 0.52` 才用亮字、饱和度夹在 `0.42~0.78`——地形跟着这份颜色走就必然比原项目暗一档灰一档。**直接把歌词那份换成上游公式是红线**（「能不动 UI 就不动 UI」，那会连歌词可读性一起改）。
- 解法是**并行第二份调色板**，同一次封面扫描算两遍：新增 `lyricGroundPaletteFromHsl()`（`:8453`，内部走 `lyricHighImpactTextHsl(hsl, {avgL: avgL, minS: 0.90})`，饱和顶 `0.90`、亮度顶 `0.90`、`shadow 'rgba(0,6,10,0.48)'`、`glow` 走 `rgbCss(c1, 0.30)`），结果**只**写进新字段 `groundPrimary` / `groundSecondary` / `groundHighlight`，`public/sonic-topography-preset.js` 只读这三个（`palette.groundPrimary || palette.primary` 兜底）。**歌词那五个字段 `primary/secondary/highlight/shadow/glow` 一个字节没动**，测试逐字节回归过。以后也不要「顺手统一」两份调色板——它们服务的是两个互相矛盾的目标（可读性 vs 忠实观感）。
- 地形支的兜底门槛照上游七条（`monochrome === true` / `avgL < 0.16` / `sampleChroma < 0.055` / `avgChroma < 0.026` / `maxChroma < 0.095` / `colorfulRatio < 0.014` / `hsl.s < 0.060`），另加暗色调红紫封面那条，踩到任意一条退 `silverBlueLyricPalette()`。**`lyricHighImpactTextHsl`（`:8431`）的默认 `minS` 是 `0.88`，地形支显式覆写成 `0.90`**；`neutralCutoff 0.035`、亮路径夹 `[0.66, 0.94]`、暗路径 `max(l + 0.30, 0.74)` 夹 `[0.70, 0.90]`、`sampledBright: avgL > 0.66 || hsl.l > 0.62`。
- **一处历史怪癖，特别容易被当 bug「改掉」**：`updateLyricPaletteFromCover`（`:8607`）给文字支传的第三个入参是 `Math.max(0, best.score)`（打分值 `chroma*1.6 + (0.5-|lum-0.5|)*0.45` 当彩度用），给地形支传的是**真彩度**。所以灰封面下歌词算出偏红的暖灰（`rgb(217,161,161)` / `rgb(193,157,123)` / `rgb(229,213,209)`）、地形正确地退银蓝。**两边确切输出都被 `tests/lyric-cover-palette-split.test.js` 钉住了，别把 `best.score` 换成 `chroma` 去「对齐」。** 写这条测试时我自己差点断言错（以为灰封面歌词也退银蓝），是手算了一遍 `score = 0.2118` 才发现。
- **差异 ⑤：预设 8 的配色喂错了源。** 上游给壁纸层的是原始封面取色，本仓库喂的是给歌词做过可读性调整的颜色（抬亮、压饱和），整幅画面偏灰偏亮。这一版 `updateLyricPaletteFromCover` 追加 `rawPrimary` / `rawWarm` / `rawCool` / `rawLight` / `rawDark` / `rawAccent` / `rawAverage`，再加一层按**占地面积**取色的 `rawAreaPrimary` / `rawAreaBase` / `rawAreaWarm` / `rawAreaCool` / `rawAreaLight` / `rawAreaAccent`（`lyricCoverAreaPaletteFromBuckets`，`:8563`：`24` 一档把采样点丢进颜色桶、桶内点数即面积权重、先按点数再按彩度排、六个角色按 `avoid` / `minDistance` 挑、`colors` 上限 10 色），`workshopCustomThemeForRegions` 按 **`rawArea*` → `raw*` → 歌词色**三级取。同时**壁纸层不再读 `visualTintColor`**：配色只跟封面走，和原作一致（`doesNotMatch(/visualTint/)`）。
- **差异 ⑥（并入 ⑤ 一起改）：`gridSize` 跟画质档位联动是本机适配，不是原作。** 原作的地形密度、涟漪半径、流星尺度都是照 `320` 调的，一改网格数整幅画面比例就变。删掉 `QUALITY_GRID_SIZE`，固定回原作 `project.json` 的 `320`（`workshopid 3747222633`），五个画质档都一样，`doesNotMatch(/QUALITY_GRID_SIZE/)` 钉住。**上一节那条「本机适配只有三处」到这一版只剩「地形基色自己压暗」与 `showPlayerController: false`。**
- **测试技巧一：大括号配平的源码切片器。** `tests/lyric-cover-palette-split.test.js` 里的 `functionSource(name)` 从 `public/app.js` 按 `^function <name>\b` 定位，然后逐字符配平大括号，**期间跳过 `//` 行注释、`/* */` 块注释和 `' " \`` 三种字符串**（不跳的话字符串里的花括号会让计数错乱），把 17 个调色函数拼进一个 `vm.runInNewContext`。**不要为了测调色板去整份读 `app.js`**（4 万多行；本轮一次整份 `Read` 被用户当场否掉）。
- **测试技巧二：跨 vm realm 的 `deepStrictEqual` 对结构相同的对象会判不等**（intrinsics 不同）。断言前先 `JSON.parse(JSON.stringify(v))` 拷回本 realm，或直接比 `JSON.stringify`。另外监视器内部有 `followValue` 平滑，**对单帧的 band 值做精确断言必然是错的**——要么连跑多帧、要么只断言单调性。
- 造桶测试的一个坑：颜色按 20 递增会让两个颜色哈希进同一个 24 档桶（还会超过 255），要测 10 色上限就用 `4 + i * 24`（`i = 0..10`，11 个互不相同的桶）。
- 验证：全量回归 **`987/987`** 通过（v1.8.9 基线 `965`，新增 `tests/sonic-audio-monitor.test.js` 12 例 + `tests/lyric-cover-palette-split.test.js` 9 例，另扩写 `tests/sonic-topography-preset.test.js` 到 14 例、`tests/sonic-workshop-preset.test.js` 到 29 例）；`node --check` 过四个改动过的 `public/*.js`。三条负向守卫 `doesNotMatch(/QUALITY_GRID_SIZE/)` / `doesNotMatch(/visualTint/)` / `doesNotMatch(/beatPulse \* 1\.35/)` 专门防本机适配长回来。
- **已知边界（照实说）：结论全部来自逐项比对上游源码加测试，没有在真实窗口里逐帧对着原项目比过观感。** 「像不像」最终只能人眼判；再报「不像」时需要的是具体描述（哪个预设、哪个部位、快歌还是慢歌），而不是再核一遍源码。
- 过程上的两条教训留着：**用户明确要求「不要跑多个agent」**（本轮开头起了一个 13 agent 的 Workflow 被否掉，已记进 `memory/no-multi-agent-fanout.md`）；**整份 `Read` `public/app.js` 会被否**，对那个文件的检查一律走 Grep / `offset+limit` / `sed` 范围。

### 2026-09-05 - 修好预设 8 没在放歌时糊成惨白（v1.9.1）

- 用户报的是一句话：「「音域回响 Wallpaper Engine」（原作 CmzYa）这个预设无法正常显示」。**是 v1.9.0 自己引入的回归**，不是 v1.8.9 那份 vendor 产物的问题。
- **根因：`workshopCoverHex` 的取色链末尾留着歌词文字三色。** v1.9.0 的差异 ⑤ 把预设 8 改成吃 `rawArea*` / `raw*` 是对的，但链尾照抄了上游的 `pal.primary || pal.highlight || pal.secondary` —— 上游拿同一份调色板既画歌词又画地形，**本仓库这三个字段是歌词文字色**，为了压在封面上读得清一律抬过亮度，而且 `stageLyrics.coverPalette` 的**初值**（`public/app.js:5706`，`palette` 在 `:5699`）就是近白的 `#d6f8ff` / `#9cffdf` / `#eef7ff`。于是**没在放歌 / 这首歌不带封面 / 封面取色还没跑完**这三种常态下，八个壁纸 uniform 一起被顶到近白：`uBaseColor2` 亮度 `0.91`，`uCoolCore` / `uRippleColor` / `uPeakColor` 三个还挤成同一个 `#eef7ff` —— 画面没了明暗层次，就是「一片惨白的雾」。**这条链只在有封面色时被 `rawArea*` / `raw*` 抢先，所以 v1.9.0 发版时肉眼看播放中的画面是正常的，defect 藏在停播态。**
- **修法（最小、且比上游更保守）：链尾只留 `rawArea*` → `raw*`，一个都没有就落在原作自己的字面量上** —— `base #16060f` / `cool·peak #99c4ff` / `ripple #f8d8ff` / `primary·warm #cb6c89`，正好复现原作默认主题 `coral-mirage`。有封面色时的取色顺序和上游逐项一致，播放中的画面和 v1.9.0 完全一样。
- **坑一：别把合法的 raw 尾巴一起删掉。** 我第一版顺手连 `|| pal.rawAreaPrimary || pal.rawPrimary` 也删了，单色封面就退不到主色（`single.uCoolCore` 从 `#7ef9ff` 掉回兜底 `#99c4ff`），测试立刻红。角色链只砍歌词那一档。
- **坑二：别给测试加「所有 uniform 亮度 ≤ 0.9」这种通用断言。** `uRippleColor #f8d8ff` 本身亮度 ≈ `0.902`，那是原作的合法亮色。换成有针对性的判据：`uBaseColor1 < 0.05`、`uBaseColor2 < 0.4`（惨白现场 `0.91`）、cool≠ripple 与 warm≠cool 的撞色检查、`uCoolCore` 与 `uBaseColor1` 至少 `0.3` 的亮度落差，再加源码级 `doesNotMatch(/pal\.primary|pal\.highlight|pal\.secondary/)`。新测试还把 `app.js` 里 `coverPalette` 初值的五个字面量逐字钉住 —— 哪天有人把初值改成深色，这条会红，提醒一起复核这套兜底。
- **预设 7 有同样的歌词兜底，这一版故意不动。** `public/sonic-topography-preset.js:181-197` 的 `sonicCoverGroundTheme` 也写着 `palette.groundPrimary || palette.primary`，但它的 `base1 = primary.clone().lerp(new THREE.Color(DEFAULT_GROUND_BASE_COLOR), 0.84)` 把基面强行往深色拉 84%，歌词色顶不亮基面，最多掉点饱和 —— 不是同一个病，别顺手「一起修」。
- **排障顺序留个记录，下次遇到「预设 8 不显示」直接从最后一条开始。** 依次核过并**排除**：vendor 四文件在包里且 MIME 正确、`assets/index-Z-j1MQ-r.js` 确实带着 `mineradio-custom` 那两处补丁且未知主题会退 `Sd.nocturnal`、全仓库没有 CSP 也没改 `webSecurity`、渲染器走 HTTP 所以 `type="module"` 能加载、`#sonic-workshop-layer` 的 `z-index:0` 在 `#canvas-container`（`1`）之下且主渲染器 `alpha:true` + `setClearColor(0x000000,0)` + `scene.background = null`、iframe 尺寸靠 bundle 自带的 `ResizeObserver` 自愈、脚本顺序正确、全 `public/` 只有一处写 `coverPalette`。**最快的一步是把推下去的 8 个 uniform 读回来看**，一眼就能看出它们全挤在近白。
- **两条环境坑（不是产品 bug）：** 预览窗格在后台会把 `document.hidden` 置真 → `body.render-deep-sleep`（`public/app.css:121`）把整层算成 `opacity:0`，且 iframe 画布卡在 `300×150`；用 `!important` 强改不管用，因为 `update()` 每帧重写 `layer.style.opacity`、`ensureLayer()` 还会重置成 `'0'`。办法是先截一次图把窗格拉到前台，再等那 `520ms` 淡入跑完，画布自己会长回 `1280×720`。另外**用户的播放器正开着 5 个 `Mineradio.exe`，`desktop/main.js:245` 的 `requestSingleInstanceLock()` 让 Electron 侧复现走不通，绝不能杀进程去腾位置**。
- 修完读回的一组正常值（供下次比对）：`uBaseColor1 #040103`、`uBaseColor2 #3e1c2a`、`uCoolCore #99c4ff`、`uCoolEdge #495e7a`、`uWarmCore #cb6c89`、`uWarmEdge #723c4d`、`uRippleColor #f8d8ff`、`uPeakColor #99c4ff`、`uGlowIntensity 0.758`。
- 验证：全量回归 **`988/988`** 通过（v1.9.0 基线 `987`，新增 1 例；另修正 `single.uRippleColor` 的旧期望值 —— 它原先等于 `#7ef9ff` 只是因为走了那条歌词兜底，现在正确地等于原作的 `#f8d8ff`）。`node --check` 干净。**这一版的画面是在真实渲染里看过的**，不像 v1.9.0 那样只有源码比对。
- **发布环节新记一条（第十次双草稿之外的新坑）：`make_latest` 和 `draft` 挤在同一次 PATCH 里会不生效。** 这一轮 `gh api -X PATCH …/releases/<id> -f draft=false -f make_latest=true` 返回 200，但 `releases/latest` 仍指着 `v1.9.0`；改用 UTF-8 JSON 文件 `--input`（`{"draft": false, "make_latest": "true", "name": …, "body": …}`）再 PATCH 一次才切过去。**发完一定要复查 `gh api repos/oirge/Mineradio/releases/latest`** —— 不切的话老客户端的更新检查看不到新版，等于白发。工作流建出来的 Release 名字只有裸版本号（这次是 `1.9.1`），标题与正文都得在这次 PATCH 里一起写。

### 2026-09-05 - 修好预设 8 在客户端里整幅全黑（v1.9.2）

- 用户给的是一句话加两张截图：「这个是黑的还是没修好啊」 —— 视觉控制台里预设 8 已选中（红圈标出），整个背景纯黑。**这不是 v1.9.1 没修好，是另一个缺陷**：惨白是配色问题（在浏览器里验的），全黑发生在更前面一步 —— 壁纸那一页根本没被允许加载。
- **根因：`desktop/main.js` 的主窗口导航守卫拦掉了预设 8 自己的 iframe。** `installMainWindowNavigationGuard` 在 `will-frame-navigate` 上的策略写着「一切子 frame 导航一律拦截」，而预设 8 的整幅画面就跑在 `<iframe src="vendor/sonic-workshop/mineradio-bridge.html">` 里（`public/sonic-workshop-preset.js:25` 的 `BRIDGE_SRC`）。iframe 永远停在 `about:blank`，`public/app.css:110-111` 给层和 iframe 都写了 `background:#000`，选这个预设时封面粒子整层又是收起的 —— 拼起来就是纯黑。
- **守卫是 `cd1a75a`（v1.5.2）加的、`f8d624e` 加固过，比预设 8（v1.8.9）早四个版本 —— 也就是说这个预设在打包客户端里一次都没显示过。** v1.8.9 / v1.9.0 / v1.9.1 三版对它的调色改动，用户在客户端里一个都看不见，只有浏览器跑源码时生效过。下次「某个 iframe 特性在客户端里没效果」先看主进程守卫。
- **一起修掉的签名错：Electron 派发 `will-frame-navigate` 的实测形状是 `(event, url, isSameDocument, isMainFrame, processId, routingId)`，而第一个参数既是事件也是详情**（自带 `url` / `isSameDocument` / `isMainFrame` / `processId` / `routingId` / `frame`（`WebFrameMain`）/ `initiator` / `preventDefault`）。老代码按 `(event, details)` 从**第二个**参数读详情，那里其实是 url 字符串，于是 `details.frame` 恒 `null`、`details.isMainFrame === true` 恒假，一路走到末尾 `event.preventDefault()` —— 无论目标是哪儿都拦，连主 frame 指向本地 `/index.html` 也拦（应用是 SPA、初始 `loadURL` 不触发导航事件，所以主 frame 那一半的错拦一直没人撞上）。
- **最贵的一课：旧测试是按「详情对象在第二个参数」这个 Electron 不存在的签名调的，所以它一直是绿的。** 假签名的测试比没有测试更坏 —— 它让「守卫工作正常」这个结论有了证据的外观。以后给 Electron 事件写测试，先在真实 Electron 里把参数打出来核实 arity 和字段挂在谁身上，再写断言。
- **改法**：两个 handler 合并成一份 `guard`（同挂 `will-navigate` 与 `will-frame-navigate`），参数先过 `readFrameNavigationDetails()` 归一化（认三种形态：事件自带字段 / 位置参数 / 旧式详情对象；`frame.parent` 在时优先用它判主子），再交 `isAllowedFrameNavigation()` 判定 —— 主 frame 只能停当前 `127.0.0.1:<mainServerPort>` 的 `/` 或 `/index.html`（`isTrustedMainDocumentUrl` 原样不动），子 frame 只放行同端口的 `/vendor/sonic-workshop/mineradio-bridge.html`（新增 `isTrustedWallpaperFrameUrl`，`pathname` **全等**比较）且必须是主 frame 的**直接**子级，其余一概拦。整个 handler 包 `try/catch`（读已销毁的 `WebFrameMain` 会抛，抛了按拦截算）。
- **坑：新 helper 必须写在测试接缝里。** `tests/main-window-navigation-ipc-trust.test.js` 的 `extractTrustCore()` 是从 `function isCurrentMainWindowSender(` 切到 `function isCurrentDesktopLyricsWindowSender(` 再 `vm.runInNewContext`，上下文只有 `{mainWindow, mainServerPort, URL}`；写到这段外面测试拿不到，在这段里引别的模块级变量会 `ReferenceError`。
- **放行只是让这一页加载出来，没有放开任何权限。** `isTrustedMainFrameSender` 仍然一见 `frame.parent` 就 `false`，所有高权限 IPC 都过它；全仓库没有任何 CSP、没有 `webRequest` 拦截、没改 `webSecurity` —— 守卫是唯一那道门，这也是为什么它一错就是整幅黑。
- **迷你播放器的 `installMiniPlayerNavigationGuard`（`desktop/main.js:3756`）有一模一样的签名错，这一版故意没动**：那边没有任何 iframe、页面走 `loadURL`，错拦不产生可见后果且偏安全侧；要改得连带改 `tests/mini-player-security-settings.test.js` 的接缝注入，与本次无关。
- **验证方式换了，这是这一版最该记住的流程改动。** v1.9.1 只在浏览器 / HTTP harness 里验过，天然验不出主进程守卫的问题 —— 于是漏掉了这个黑屏。这次在临时目录起了**独立最小 Electron 探针**：离屏渲染（`webPreferences.offscreen`，`paint` 事件拿 `NativeImage`）、独立 userData、**不申请单实例锁**（用户那 5 个 `Mineradio.exe` 正开着，绝不能碰），`vm` 装真实 `desktop/main.js` 的信任核心、起真实 `public/` 静态服务、加载真实 `index.html`，然后 A/B：旧守卫 → 子 frame `about:blank`、无 canvas、`gl:false`、亮像素 `4.38%`、平均通道 `6.66`（和用户截图一致）；新守卫 → `readyState: complete`、WebGL2 `1280×720`、亮像素 `89.0%`、平均通道 `44.78`，画面是原作默认主题 coral-mirage 的方柱地形（探针没导入歌曲，顺带复验了 v1.9.1 的兜底配色不发白）。两张 PNG 直接发给了用户。
- **探针坑：只调 `MineradioSonicWorkshop.onPresetChange(0, 8)` 不够**，得先 `window.fx.preset = 8`；否则渲染循环下一帧的 `update()` 里 `isActive(fx)`（`Number(fx.preset) === 8`）为假，立刻 `removeLayer()`，探针只会报 `no-iframe`。
- **一条对排障有用的实测：`will-frame-navigate` 对 iframe 的首次加载就会触发**（那时 `frame.url` 还是空串、`frame.parent.url` 是父页），所以 `preventDefault()` 是直接掐死初始加载，不只是掐后续跳转 —— 这就是为什么表现是「iframe 停在 about:blank」而不是「先加载出来再被弹回」。
...
- **`9733721a-009e-52bc-b705-49059cd80258` 是换身份之前本仓库留下的卸载注册表 GUID（实测取自本机 `HKCU`），原项目用的是同一个**，所以只靠这个键判断会误伤原项目的安装。**实际拉下原项目的 `build/installer.nsh` 和 Release 列表核过：原项目也写同一个 `.mineradio-install-root` 标记，也发过 `v1.1.1`，所以「版本号 1.x + 有标记」根本分不出来。** 唯一可靠的区分是 electron-builder 写进 `resources\app-update.yml` 的发布源：我们是 `owner: oirge`，原项目是 `owner: XxHuberrr`。`installer.nsh` 的 `MineradioOfferLegacyUninstall` 三道门禁：目录里有安装标记、`app-update.yml` 里有 `owner: oirge`（`MineradioLegacyInstallIsOurs` 逐行读）、新目录不在旧目录里面；最后仍是 `MB_DEFBUTTON2` + `/SD IDNO` 由用户点头，且绝不传删数据的开关。**electron-builder 怎么从 appId 推出这个 GUID 一直没复现出来，所以只硬编码了实测到的旧 GUID，不要去猜新的。**
- **安装根目录在 `HKCU\Software\<GUID>\InstallLocation`**（electron-builder 的 `initMultiUser` 写的，实测 `D:\Mineradio`），卸载记录那个 `Uninstall\<GUID>` 键下的 `InstallLocation` 是空的；先读前者，再退到 `DisplayIcon`（卸载器同目录的图标）用 `${GetParent}` 反推。
- **前缀比较要补反斜杠，这个 bug 差点让提示永远不出现**：判断「新目录是否嵌在旧目录里」时直接拿 `D:\Mineradio` 当 `$INSTDIR` 的前缀比，默认的 `D:\Mineradio-oirge` 会被判成子目录而跳过。两边都补上 `\` 再比（`D:\Mineradio\` 对 `D:\Mineradio-oirge\`）才对。静默安装里 MessageBox 被 `/SD` 自动作答、`DetailPrint` 看不见，所以加了探针：环境变量 `MINERADIO_INSTALLER_LEGACY_PROBE=<文件>` 存在时，每一步结论追加写入该文件，真机上就能核对走到了哪一步。
- **叶子名是 `Mineradio` 的目录一律改到旁边**（`MineradioNormalizeInstallDir`：`D:\Mineradio` → `D:\Mineradio-oirge`），绝不嵌进去 —— 旧版（1.9.3 及更早）的卸载器是 electron-builder 默认的 `RMDir /r $INSTDIR`，嵌进去会被连带删掉。原项目的安装器走 electron-builder 默认的按镜像名结束进程（`Mineradio.exe`），碰不到 `Mineradio-oirge.exe`；应用内更新器启动安装器只带 `--updated`、不带 `/S` 和 `/D=`（`desktop/update-installer-launcher.js`），所以从 1.9.3 升上来会看到完整安装界面和这个弹窗。
- **`build/after-pack.js` 改按 `platformSpecificBuildOptions.executableName` 找 exe**（逐个 `existsSync` 兜底）。本地打包实测 electron-builder 的 `productFilename` 其实也跟着 `executableName` 走（卸载器叫 `Uninstall Mineradio-oirge.exe`），所以旧写法不会挂，这只是不再依赖它的内部约定；`FileDescription` / `ProductName` 跟 `productName`，任务管理器里才和原项目分得开。
- **本机真机 E2E 留下的事实**：新身份在这台机器上生成的卸载 GUID 是 `423fa7af-3a0d-514c-9fef-ebee5546ae66`（`HKCU\Software\<GUID>\InstallLocation` 同样由 electron-builder 写入）；`Setup.exe /S /D=D:\tmp\coexist-e2e` 实际装到 `D:\tmp\coexist-e2e\Mineradio-oirge`，装出 `Mineradio-oirge.exe` + `Uninstall Mineradio-oirge.exe`，桌面和开始菜单各一个 `Mineradio 二创.lnk`；探针记录 `legacy=prompt root=D:\Mineradio version=1.9.3` → `legacy=declined`；`Uninstall Mineradio-oirge.exe /S /currentuser` 之后目录、注册表、快捷方式全清，旧安装与 `%APPDATA%\Mineradio-oirge` 原样。**本地打包必须加 `-c.npmRebuild=false`**（没有 Visual Studio，`uiohook-napi` 自带 prebuilds），Electron 二进制要单独 `node node_modules/electron/install.js`。
