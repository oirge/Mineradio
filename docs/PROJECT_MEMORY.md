...
- 当前源码检查点：`v1.2.48`，当前 HEAD 为 `686e446`（`release: finalize 1.2.48 desktop lyrics poller orphan fix`）；tag `v1.2.48` 指向同一发布提交 `686e446`，修复提交为 `93a234e`。
- 当前工作分支：`codex/release-1.2.44-memory`（已推送到远端同名分支）。
- 最近正式安装包 Release 基线：`v1.2.48`（tag 提交 `686e446`，GitHub Releases 已标记 Latest，4 资产齐全：Setup.exe/.blockmap/latest.yml/`1.2.47 -> 1.2.48` 快速补丁，补丁仅含 `desktop/main.js`）；远端 `main` 不是本轮发布基线，仍停在 `v1.2.34`。
