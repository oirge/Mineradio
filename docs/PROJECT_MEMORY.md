...
- 当前源码续版：`v1.8.3`（QRC 加密歌词 + 歌词格式兼容增强：QRC 密文（十六进制文本与二进制两种载体）能解、同名多份歌词按 `LOCAL_LYRIC_FORMAT_RANK` 挑且可在自定义歌词弹窗里手动选并记住、歌词编码自动识别、时间轴在野写法修复。**QRC 的 3DES 不是标准 3DES**（五处刻意保留的在野偏差），`des-ede3` 不能当参考实现，正确性靠 `tests/lyric-format-hardening.test.js` 的已知答案向量钉住；解密链 `D(K3) → E(K2) → D(K1)` → zlib → 剥 BOM，三个密钥与 `KRC_XOR_KEY` 一样是公开格式常量。`.qrc` 的 MIME 必须 `application/octet-stream`（`server.js` 与 `desktop/main.js` 两处）。优先级只在「精确路径 → 同目录同名 → 模糊名」同一层内比；候选桶查表必须走 `localLyricCandidateBucket()`；合法 UTF-8 绝不重猜；`[offset:±N]` 正值表示提前、偏过头夹到 0；手动选歌词走 `propagateLocalLyricPick` 且 `localLyricCacheHydrated` 置 `true`。逐条约束与踩坑见本文件 `### 2026-09-04 - v1.8.3` 小节，AGENTS.md 的当前源码续版条目是同一份的完整版，发布过程见 `RELEASE.md` 顶部小节。全量回归 `881/881`（上一版基线 `871`）。**上一版 `v1.8.2` 的多格式歌词约束（分流顺序即语义，KRC / QRC 必须排在 YRC 前面；新后缀四处清单；`finalizeLyricLineDurations` 第二参数）整段继续有效，见本文件 `### 2026-09-03 - v1.8.2` 小节。** **更早 `v1.8.1` 的整机备份约束整段继续有效：**整机备份：设置面板「高级」页「备份」折叠区导出 `mineradio.backup`、在新电脑导入。落盘就是用户给的四段 `{version:2, database:{songs,playlists,favorites,history}, config:{theme,eq,player}, paths:{musicFolders}}` 外加一段 `meta`，键序被测试用 `deepEqual(Object.keys(...))` 钉死。**歌曲身份存 `{folder, rel, size, mtime}`、绝不存绝对路径**：本机 `localKey` = `绝对路径:size:mtime`，歌单/收藏引用键与 SQLite `song_key` 都由它派生，存绝对路径换机盘符一变全体扑空；导入按新根重拼并重算这四样。`mineradioBackupRelPath` 必须保原始大小写（用归一化结果只算长度、从原始串尾部切；`normalizeLocalLibraryPathKey` 会 `toLowerCase()`，拿它输出当 `rel` 连同机导入都复不出原 `localKey`），分隔符由 `mineradioBackupPathSeparator` 看根路径决定。**探「备份里的音乐文件夹在不在本机」只能用 `api.refreshLocalMusicFiles(folder, [])`（空数组＝只验目录+登记授权根），绝不能用 `readLocalFileRange`**：授权表在新会话是空的、必然 `LOCAL_FILE_NOT_AUTHORIZED`，会把同机导入误判成换机。用户在重选目录时取消一律 `return null` 整个导入放弃，**绝不能返回 `[]`**（那会被当成「没有音乐文件夹」继续覆盖歌单）。`config.player` 是 `MINERADIO_BACKUP_PLAYER_KEYS` 十六键白名单，导出导入两侧都只认这些键（挡住手改备份往 localStorage 塞任意键，含 `__proto__`）。导入抹掉五个临时键：曲库快照/索引走 `localStorage.removeItem`，队列快照/会话/续播位置走 `removePersistentLocalStorageItem`（要连 `desktop-ui-state.json` 镜像一起清）。播放统计复用累加语义的 `bumpPlayStat`，所以**同机重复导入会把次数翻倍**——这就是导入要两步确认（`12s` 内点第二次）的原因；`lastPlayedAt` 统一垫 `1` 而不是 `0`，因为主进程是 `|| Date.now()`。历史 `cover` 一律清成 `''`（属点名不要的大封面缓存），上限 `180` 取自本机 `compactListenStatsHistory(null, …, 180)`。音效档案真身在 IndexedDB 用户态 `user-fx-archives`，导入要写用户态并删 localStorage 旧键。导入尾巴是 `setTimeout(restart, 900)` 整体重启，不做热重载。两个主进程 handler 套 `trustedMainFrameHandler` 且**必须留在 `mineradio-export-json-file` 之前**（否则污染 `tests/import-json-file-size-limit.test.js` 的切片）。备份块夹在 `var MINERADIO_BACKUP_VERSION = 2;` 与 `function openPluginManager() {` 之间跑 vm，里面新增裸标识符会 `ReferenceError`。界面只加 `fx-backup-fold`（`fx-plugin-fold` 与 `fx-advanced` 之间，`public/app.css` 一行未动）；**它故意没进 `fxPanelTargetForNode`，落到「高级」页靠的是 `organizeFxPanel` 里 `current` 的继承，被挪到非 advanced 折叠区上面就会静默跑到别的页**。默认不备份：音频、封面缓存（含每首歌自定义封面/节拍图/歌词）、临时文件。`tests/mineradio-backup-export-import.test.js` 18 例，全量回归 `864/864`；发布前只跑过 Electron 启动冒烟（干净开到首页、`app.js` 零报错），**备份面板落位、真实文件对话框与导入后重启仍未肉眼验证**。）
- 上一续版：`v1.7.29`（同步指示器「已同步 xx 首歌曲」移到搜索框下面并接上主题插件令牌；另修掉曲库清理的扫描代号缺陷——`desktop/local-library-store.js` 的 `nextSyncStamp` 因为清理用 `seen_at<>?` 判定，**必须每根严格递增、不能直接用 `Date.now()`**，同一时钟刻度里的两次扫描会让该删的文件躲过清理。全量回归 `846/846`。）
