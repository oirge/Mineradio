...
- 当前源码检查点：`v1.2.50`，当前 HEAD 为 `68d5485`（`release: finalize 1.2.50 track switch listen session guard`）；tag `v1.2.50` 指向同一发布提交 `68d5485`，修复提交为 `9114a79`。
- 当前工作分支：`codex/release-1.2.44-memory`（已推送到远端同名分支）。
- 最近正式安装包 Release 基线：`v1.2.50`（tag 提交 `68d5485`，GitHub Releases 已标记 Latest，4 资产齐全：Setup.exe/.blockmap/latest.yml/`1.2.49 -> 1.2.50` 快速补丁，补丁仅含 `public/index.html`）；远端 `main` 不是本轮发布基线，仍停在 `v1.2.34`。
