...
- 当前源码续版：`v1.8.7`（① 新增第 8 个视觉预设「音域回响」= 预设 7，`SPECTRUM_ECHO_PRESET_INDEX = 7`。**CmzYa 的原壁纸源码不在本机**（全盘扫 Steam Workshop `431960` 零匹配，WebSearch 也定位不到），加上搬 Workshop 作品有授权/署名问题，所以这是在 Mineradio 自己 shader 框架里的**原创同名预设**，`presetMeta` 写 `频谱回响 · 致敬 CmzYa`，不是移植；另一条路本来就通——项目已有 Wallpaper Engine 集成（`desktop/wallpaper-engine-library.js`，`WALLPAPER_ENGINE_APP_ID = '431960'`），装了原作就能直接当桌面壁纸层或播放器背景板跑。**技术核心是新数据通路**：着色器原本只有 `uBass/uMid/uTreble/uBeat/uEnergy` 五个标量、做不出音域，于是加了 64 频段 × 64 历史行的 RGBA8 `DataTexture`（`spectrumEchoTex`），每次推进先 `copyWithin` 把旧行整体下移再把当前频谱写进第 0 行，**故意不用环形索引**（`LinearFilter` 会在环绕处插出一道缝），`DataTexture.flipY = false` 所以纹理 `v = 0` 就是最新行；通道 R 归一幅度 / G 起振快收尾慢的包络 / B 瞬态 / A 该行节拍能量。着色器角度当音域（`bandN = abs(aUv.x * 2 - 1)`，低音正下方、两侧升到高音、首尾无缝）、半径当时间（`age = lane / 0.86`），`decay = pow(1 - age, 1.35)`、`pos.z = -age * 3.9 + ...` 沉成漏斗，外圈 14% lane 是只吃高频的空气尘。分频 `ensureSpectrumEchoBins` 从 bin 2 起、上界 `len * 0.55`、几何步长铺 64 段（严格递增、每段至少一个 bin）；`SPECTRUM_ECHO_PUSH_INTERVAL = 1/48`，**掉帧最多补一行、不做追赶**。`updateSpectrumEchoField` 第一行 `if (!fx || fx.preset !== SPECTRUM_ECHO_PRESET_INDEX) return;`，其他 7 个预设零开销；进出都 `resetSpectrumEchoField()`，否则再进会闪一帧陈旧频谱。着色器三条硬约束：星河/安魂收成 `else if (uPreset < 6.5)`、预设 7 用最后的 `else`；`vBright` 与 `gl_PointSize` 里 `uPreset > 6.5` **必须排在 `> 4.5` 之前**；`vinylHiResGuard` 用 `step(3.5) * (1 - step(4.5))` 夹住不许溢到 7。**新 sampler 只能挂在 `uniform sampler2D uCoverTex, uPrevCoverTex, uEdgeTex, uRippleTex, uSpectrumTex;` 那行**——`bloomVs` 是由 `vs` 两次精确字符串替换派生的，`'uniform float uMouseActive, uPixel, uColorMixT, uLoading;'` 与 `'gl_PointSize = sz * uPixel * uPointScale;'` 必须逐字节不变。**本轮唯一真实踩坑：`var len = frequencyData.length;` 在 `public/app.js` 里必须唯一**——`tests/audio-analysis-hot-path.test.js:139` 拿它当切片起锚，我第一版在 `updateSpectrumEchoField` 里也声明了同名局部量且位置更靠前，`indexOf` 切到我的函数上，那条常驻测试的「单次扫描只能有一个 for」当场变成 2，改名 `binCount` 才修好；帧钩子也因此只放在 `uniforms.uEnergy.value = audioEnergy;` 之后。转场 `wallpaperFlow` 提成 `isSoftFlowPreset(preset)` 覆盖 5 和 7（连续场只给极轻一下：`duration 0.30` / `uBurstAmt 0.05` / `camPunch 0.04`）；`setPreset` 给 `p === 7` 的基线 radius `8.6` / phi `0.16` / theta `0`，`orbit.baselineTheta` 那行没动。② **全屏退出黑屏卡顿**根因是遮罩被后到的信号一路顺延（退出时 resize、边界还原、`leave-full-screen` 状态推送连发好几轮，每轮都重排回亮计时器），现在 `scheduleFullscreenTransitionReveal` 只允许提前——`if (revealDue && due >= revealDue) return;`，resize 或尺寸已跳变用 50ms、纯状态推送用 110ms；硬上限 `FULLSCREEN_TRANSITION_MAX_COVER_MS = 320` **独占 `deadlineTimer` 一个槽**，resize/state 只能重排 `revealTimer`、抢不掉它，动作前摇 110ms、收尾 220ms、降低动效偏好 20/90/30/80；`desktop/main.js` 的 `applyWindowedBounds` 边界已到位就不再 `setBounds` 但仍 `sendWindowState`，`leave-full-screen` 与 `leave-html-full-screen` 都立刻推状态；CSS **窗口壳过渡不能带 `filter`**（`#desktop-window-shell` 承载 WebGL 画布，加 filter 会让整窗每帧重新合成），只留 `will-change:transform`。测试 `tests/spectrum-echo-preset.test.js` 9 例 + `tests/fullscreen-exit-transition.test.js` 7 例，全量 `932/932`（v1.8.6 基线 `916`）；着色器在 Electron + SwiftShader 上真编译链接过、`uSpectrumTex` 是 ACTIVE_UNIFORM；**但真实窗口里没有肉眼看过**。逐条见 `docs/PROJECT_MEMORY.md` 的 `### 2026-09-05 - v1.8.7` 小节，发布过程见 `RELEASE.md` 顶部小节。）
...
### 2026-09-05 - v1.8.7 音域回响：原作源码不在本机，就在自己的框架里做等效，别假装是移植

- 用户原话（issue 形状，两件事）：「增加视觉预设：希望增加音域回响视觉预设（Wallpaper Engine)(作者CmzYa）。全屏模式退出时，有明显的黑屏卡顿问题。」建议里写「可以从原项目的项目代码去接入实现」。
- **先说结论：接不了原代码。** 后台把 `/c /d /e /f` 全盘扫过找 Steam Workshop 目录 `431960`，零匹配；WebSearch 也没能定位到这件作品。直接搬 Workshop 作品的代码/素材还有授权与署名问题。所以做的是**在 Mineradio 自己 shader 框架里的原创同名预设**，`presetMeta` 里写 `频谱回响 · 致敬 CmzYa`——**以后别把它描述成移植或接入原项目代码。**
