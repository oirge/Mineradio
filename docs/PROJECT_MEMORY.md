...
- **本机适配只有三处，都写在模块文件头，别当成移植错漏「修回去」**：① 配色取 `stageLyrics.coverPalette` 的 `primary/secondary/highlight`，角色分工与预设 7 对齐（`uCoolCore`=主色、`uWarmCore`=副色、`uRippleColor`/`uPeakColor`=高光色），认不出退回 `#62d6ff`/`#ff8f4a`/`#33e6ff`；② 地形基色自己压暗（`base = mixHex('#05070c', primary, 0.16)`），不跟封面副色走——本仓库的调色板是给歌词用的、没有暗部区域色；③ `gridSize` 跟画质档位联动 `{eco:224, balanced:288, high:320, ultra:384}`，**默认档 `high` = 320 = 原作 `project.json` 的默认值，开箱即与原作一致**。`showPlayerController` 保持 `false`（播放器 UI 是本仓库自己的）、`showAlbumCover` `true`。
...
- 验证：全量回归 `965/965` 通过（v1.8.8 基线 `939`，新增 26 例）。**壁纸层的实际观感没有在真实窗口里逐帧核对过**——「跟着封面换色好不好看」「频谱幅度合不合适」只能靠肉眼，发 v1.8.9 时仍是未确认状态。用户的指令是「更新好后发布新版」「能发布就一起发布」，所以按移植的忠实度发出去、留给反馈再调。
