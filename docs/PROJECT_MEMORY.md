...
- 当前源码检查点：`v1.2.49`，当前 HEAD 为 `f047f4e`（`release: finalize 1.2.49 archive import size limit`）；tag `v1.2.49` 指向同一发布提交 `f047f4e`，修复提交为 `57c282d`。
- 当前工作分支：`codex/release-1.2.44-memory`（已推送到远端同名分支）。
- 最近正式安装包 Release 基线：`v1.2.49`（tag 提交 `f047f4e`，GitHub Releases 已标记 Latest，4 资产齐全：Setup.exe/.blockmap/latest.yml/`1.2.48 -> 1.2.49` 快速补丁，补丁仅含 `desktop/main.js`）；远端 `main` 不是本轮发布基线，仍停在 `v1.2.34`。
