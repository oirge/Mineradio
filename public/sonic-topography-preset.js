/**
 * 音域回响 (Sonic Topography) — 视觉预设 7 的地形层。
 *
 * 视觉算法移植自上游 XxHuberrr/Mineradio 的 public/sonic-topography-preset.js (GPL-3.0),
 * 上游又标注移植自 yin-yizhen/sonic-topography 1.1.1 (commit 3ff303e);
...
(function (global) {
  ...
  /**
   * 把封面调色板里的任意颜色写法归一成 #rrggbb, 认不出来就退回默认色。
   * @returns {string}
   */
  function sonicPaletteHex(value, fallback) {
    ...
    if (rgb) {
      return '#' + [rgb[1], rgb[2], rgb[3]].map(function (part) {
        var n = Math.round(clamp(Number(part) || 0, 0, 255)).toString(16);
  ...
  function sonicCoverGroundTheme(fx) {
    ...
    return {
      base1: base1,
      base2: base2,
      coolCore: primary.clone().lerp(new THREE.Color('#ffffff'), 0.08),
      warmCore: secondary.clone().lerp(new THREE.Color('#ffb15a'), 0.18),
      ripple: highlight.clone().lerp(new THREE.Color('#ffffff'), 0.10)
    };
  }

  function sonicCustomGroundTheme(fx) {
    var tint = new THREE.Color((fx && fx.visualTintColor) || '#62d6ff');
    var tintMix = fx && fx.visualTintMode === 'custom';
    ...
    return {
      ...
      coolCore: new THREE.Color(sonicHex(fx, 'sonicGroundCoolColor', DEFAULT_GROUND_COOL_COLOR)).lerp(tint, tintMix ? 0.08 : 0.0),
      warmCore: new THREE.Color(sonicHex(fx, 'sonicGroundWarmColor', DEFAULT_GROUND_WARM_COLOR)).lerp(tint, tintMix ? 0.05 : 0.0),
  ...
  /** 低频两段跟着 kick 走, 上限 MAX_KICK_DEFORM 防止地形被打穿。 */
  ...
  /** 地形柱顶点着色器: simplex 噪声底噪 + 五段抬升 + 十道涟漪。 */
  function buildTerrainVertexShader() {
    return [
      ...
      '      float lifeFade=1.0-smoothstep(' + RIPPLE_SOFT_FADE_START.toFixed(2) + ',' + RIPPLE_LIFETIME.toFixed(2) + ',timeSince);',
  ...
  /** 地形柱片元着色器: 顶面负责闪烁与高光, 侧面负责体积渐变, 远处并入雾色。 */
  ...
  /** 悬浮方块只借用地形的片元着色器, 顶点端把脉冲塞进 varying 冒充高度。 */
  ...
  /** 流星与尾迹用最朴素的 MeshBasicMaterial, 初始全部缩到 0 藏在 y=-1000。 */
  ...
  /** 旋转跟着 particles.rotation 走, 和其它预设共用同一套拖拽手势。 */
  ...
  function updateSonicRotation(fx, dt, ctx) {
    ...
    if (ctx && ctx.visualRotationActive) speed *= 0.35;
    state.autoYaw += dt * speed;
  }
  /** 只在网格密度或悬浮块数量变化时重建, 其余帧复用同一批实例。 */
  ...
  function syncTerrainUniforms(fx, audio, dt, time) {
    ...
    syncTheme(state.terrainMat, fx, themeAudio, dt);
    syncRippleUniforms(time);
    if (state.floatingMat) {
      syncTheme(state.floatingMat, fx, themeAudio, dt);
  ...
  /** 底鼓触发蓝涟漪 + 偶发流星, 军鼓 / 高频触发白涟漪, 都带迟滞防抖。 */
  ...
  /** 鼠标松开时由 app.js 调用, 在指针落点打一道蓝涟漪。 */
  function pointerRipple(worldX, worldZ, strength) {
    addRipple(worldX, worldZ, strength || 1.2, false);
  }
  /**
   * 每帧入口。ctx = { scene, fx, time, visualRotation, visualRotationActive, audio }。
   * 离开预设后还会继续跑几帧把 opacity 收敛到 0, 收敛完自己隐藏并早退。
   */
