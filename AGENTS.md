...
- 遇到 `播放器性能优化`、`播放启动速度`、`点歌到出声`、`启动恢复`、`队列渲染`、`HTML 转义`、`副标题缓存`、`空搜索`、`本地曲库导入`、`本地曲库缓存`、`本地节奏缓存`、`3D 歌单详情`、`主进程本地曲库扫描`、`IndexedDB`、`封面缩略图`、`重复函数声明` 或 `requestAnimationFrame 调度` 时，必须优先读取 `.context/architecture/mineradio-player-performance-seams.md`。
