...
Mineradio 自带 **午夜靛蓝** / **暖琥珀** / **深海微光** / **暗焰余晖** / **冷杉夜雾** 五份暗色完整主题，
以及新增的浅色完整主题 **雪昼白**（六份均随安装包提供，默认不启用）。这里放的是六份自带主题的源文件，
外加一份没进安装包的 **石墨**，同时当主题写法的参考。
1.7.4 起插件只有主题一种，本目录就是插件示例的全部。
完整 API 见 [`docs/PLUGIN_AUTHORING.md`](../../docs/PLUGIN_AUTHORING.md)。

## 安装

自带的六份不用装。石墨要装：设置面板 → 插件 → 管理 → 安装插件文件，选中 `theme-graphite.json`。
一个文件就是一个插件。
...
| `theme-white.json` | 是 | 雪昼白：浅色面板 + 深色文字 + 冷蓝强调 |
| `theme-background-deep-sea.json` | 是 | 深海微光：深海蓝青完整外壳 |
| `theme-background-ember.json` | 是 | 暗焰余晖：暗红熔岩完整外壳 |
| `theme-background-forest.json` | 是 | 冷杉夜雾：冷杉绿完整外壳 |
| `theme-graphite.json` | 否 | 石墨：纯无彩，最克制的一版 |

**主题同一时刻只能启用一个。** 六份自带主题和石墨可以同时装在列表里，但启用一个的瞬间原来那个会被自动关掉，
不会出现多套完整变量互相覆盖的情况。

自带的六份卸载后不会在下次启动时被塞回来；想要回来就再手动装一次本目录下的同名文件。
...
`--th-bg-tint-opacity` 控制这层色调的透明度。所有内置主题都声明这三个变量，确保默认背景与面板外壳一起换色。
历史上的三份 `theme-background-*.json` 现在已经升级为完整主题，不再是只含三变量的最小示例。

用户在设置里选的纯色、图片、视频或透明度会通过行内 `--custom-bg-color` 压过主题；
自定义媒体、Wallpaper Engine 或「播放器背景板」启用时，封面模糊层仍按原逻辑让位。
这三个变量不会改玻璃参数、粒子、电影镜头或壁纸逻辑；它们和其它 `--th-*` 一样受主题互斥规则约束。
